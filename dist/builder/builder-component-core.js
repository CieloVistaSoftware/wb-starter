  ...
  `;
    ...
    try {
        if (window.__wb_queuedAdds && Array.isArray(window.__wb_queuedAdds) && window.__wb_queuedAdds.length) {
            ...
            for (const args of q) {
                try {
                    addComponentToCanvas.apply(null, args);
                }
                catch (err) {
                    console.warn('[wb-init] drained queued add failed', err && err.message);
                }
            }
        }
    }
...
//# sourceMappingURL=builder-component-core.js.map