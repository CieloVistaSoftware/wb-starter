...
export function table(element, options = {}) {
    ...
    if (tbody) {
        currentData = Array.from(tbody.querySelectorAll('tr')).map(tr => Array.from(tr.querySelectorAll('td')).map(td => td.textContent) // Use textContent instead of innerHTML
        );
        filteredData = [...currentData];
    }
    tableEl.classList.add('wb-table');
    if (config.striped)
        tableEl.classList.add('wb-table--striped');
    if (config.hover)
        tableEl.classList.add('wb-table--hover');
    if (config.bordered)
        tableEl.classList.add('wb-table--bordered');
    if (config.compact)
        tableEl.classList.add('wb-table--compact');
    // Search Logic
    if (searchInput) {
        searchInput.oninput = () => {
