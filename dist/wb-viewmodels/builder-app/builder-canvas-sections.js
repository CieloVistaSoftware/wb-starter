...
export function addHTMLToSection(html, section, template = null) {
    ...
    saveHistory();
    // Refresh tree
    if (window.renderComponentList)
        window.renderComponentList();
    return wrapper;
}
// Expose globally
...
export function removeCanvasElement(id) {
    ...
    if (hasChildren) {
        if (!confirm('Remove this element and all its contents?'))
            return;
    }
...
function setupCanvasInteractions(canvas) {
    ...
    canvas.addEventListener('click', (e) => {
        ...
        if (!clickedElement)
            return;
        if (e.target.closest('.el-controls, .el-btn'))
            return;
        // Deselect others
...
function injectSectionStyles() {
    if (document.getElementById('canvasSectionStyles'))
        return;
...
//# sourceMappingURL=builder-canvas-sections.js.map