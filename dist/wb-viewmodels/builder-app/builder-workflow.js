...
window.wfSaveName = () => {
    ...
    if (name) {
        ...
        if (window.toast)
            window.toast(`Workspace named: "${name}"`);
    }
    hideWorkflow();
...
window.wfSelectIntent = (intentId) => {
    ...
    if (workflow.action === 'openFullBuilder') {
        ...
        showWorkflowIndicator();
        if (window.toast)
            window.toast('Explore mode! Drag components from the sidebar.');
        return;
    }
    // For website intent, enable site builder view
    if (intentId === 'website') {
        import('./builder-template-browser.js').then(module => {
            if (module.setViewMode)
                module.setViewMode('site');
        });
    }
...
window.wfSelectType = (intentId, typeId) => {
    ...
    if (intentId === 'component' && option.sections) {
        ...
        showWorkflowIndicator();
        if (window.toast)
            window.toast(`Browse ${option.name} components in the sidebar`);
        return;
    }
    ...
    if (window.toast)
        window.toast(`Showing ${option.name} templates`);
...
function openThemeEditor() {
    ...
    if (themeSelect) {
        themeSelect.focus();
        if (window.toast)
            window.toast('Pick a theme from the dropdown');
    }
}
// =============================================================================
...
window.wfShowHelp = () => {
    document.getElementById('wfIndicatorMenu')?.classList.remove('visible');
    if (window.toast)
        window.toast('Tip: Drag components from the sidebar to build your page!');
};
// =============================================================================
// OVERLAY HELPERS
// =============================================================================
...
function injectWorkflowStyles() {
    if (document.getElementById('wfStyles'))
        return;
...
//# sourceMappingURL=builder-workflow.js.map