...
// === MODULE IMPORTS ===
import { toggleSidebar, togglePanel, switchPanelTab, toggleCategory, restoreCollapsedCategories, restorePanelTab, setupPanelKeyboardShortcuts } from './builder-panels.js';
import { showDragFeedbackUI, removeDragFeedbackUI, findInsertionPoint, createDragStartHandler, createDragEndHandler, createDragOverHandler, createDragLeaveHandler, createDropHandler } from './builder-dnd.js';
import { genId, mkEl, buildControls, add, addToGrid, addToContainer, del, dup, moveUp, moveDown, setSpan, setComponentIdCounter } from './builder-components-core.js';
import { selComp, renderProps, updP, validateComponent, changeCardType, updateElementId, getSelection, setSelection } from './builder-selection.js';
...
import { initInlineEditing, isContainer, getContainerConfig, canDropInto, findContainerFromTarget, findDropZone, showDropZoneHighlight, hideDropZoneHighlight, injectEditingStyles, initResizeHandles, addResizeHandle, initSnapGrid, injectSnapGridStyles, getEditableKey } from './builder-editing.js';
import { handleSmartDrop, applyModifier, removeModifier, getDragFeedback } from './builder-drop-handler.js';
import { loadPropertyConfig, renderPropertiesPanel, renderDOMElementProperties, getDefaultValue } from './builder-properties.js';
...
function enableBuilderInteractions() {
    ...
    const handleKeydown = (e) => {
        ...
        switch (e.key) {
            ...
            case 'g':
            case 'G':
                toggleSnapGrid(!snapEnabled);
                break;
            case 'f':
            case 'F':
                if (sel) {
                    const c = JSON.parse(sel.dataset.c);
                    toggleFavorite(c.b);
                }
                break;
            case '?':
                    ...
                    parsed.components.forEach(c => {
                        ...
                        if (c.children) {
                            ...
                            c.children.forEach(gc => {
                                ...
                                if (gc.span)
                                    setSpan(gc.id, parseInt(gc.span), saveHist);
                            });
                        }
                    });
                }
...
window.togglePreview = () => {
    ...
    if (exitBtn)
        exitBtn.style.display = isPreview ? 'block' : 'none';
