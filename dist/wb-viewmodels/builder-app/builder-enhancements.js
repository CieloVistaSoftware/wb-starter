...
class SmartComponentBrowser {
    ...
    renderComponents(components) {
        ...
        for (const comp of components) {
            if (this.searchTerm && !this.componentMatchesSearch(comp))
                continue;
            html += `
    ...
    categoryMatchesSearch(category) {
        ...
        if (category.name.toLowerCase().includes(this.searchTerm))
            return true;
        if (category.components)
            return this.componentsMatchSearch(category.components);
        if (category.subcategories) {
            return Object.values(category.subcategories)
...
function getComponentInfo(componentType) {
    for (const [catId, category] of Object.entries(WB_COMPONENT_LIBRARY)) {
        if (category.components) {
            ...
            if (found)
                return { icon: found.icon, name: found.name };
        }
        if (category.subcategories) {
...
function renderContainerComponentBrowser(containerId, searchTerm = '') {
    ...
    for (const [catId, category] of Object.entries(WB_COMPONENT_LIBRARY)) {
        ...
        if (category.subcategories) {
            for (const [subId, subcat] of Object.entries(category.subcategories)) {
                const matchingComponents = subcat.components.filter(c => !term || c.id.toLowerCase().includes(term) || c.name.toLowerCase().includes(term) || c.desc.toLowerCase().includes(term));
          ...
          `;
            ...
            const matchingComponents = category.components.filter(c => !term || c.id.toLowerCase().includes(term) || c.name.toLowerCase().includes(term) || c.desc.toLowerCase().includes(term));
            if (matchingComponents.length > 0) {
...
function addComponentToContainer(containerId, componentType) {
    ...
    if (!parentComp.data)
        parentComp.data = {};
    if (!parentComp.data.nestedComponents)
        parentComp.data.nestedComponents = [];
    // Generate unique ID
...
//# sourceMappingURL=builder-enhancements.js.map