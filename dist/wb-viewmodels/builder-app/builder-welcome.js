 ...
 */
import { PAGE_TEMPLATES, SECTION_TEMPLATES, PAGE_CATEGORIES, SECTION_CATEGORIES, expandPageTemplate, getSectionTemplate, getPageTemplatesByCategory } from './builder-templates.js';
// =============================================================================
// WELCOME SCREEN
// =============================================================================
...
function createTemplateElement(comp, id) {
    ...
    if (comp.d) {
        for (const [k, v] of Object.entries(comp.d)) {
            if (k === 'text')
                ...
                el.className = v;
            else if (k === 'src' && ['IMG', 'AUDIO', 'VIDEO'].includes(el.tagName))
                el.src = v;
            else if (k === 'href')
                el.href = v;
    ...
    if (comp.b === 'container' && comp.d) {
        ...
        if (comp.d.padding)
            el.style.padding = comp.d.padding;
        const alignMap = { start: 'flex-start', center: 'center', end: 'flex-end', stretch: 'stretch' };
        const justifyMap = { start: 'flex-start', center: 'center', end: 'flex-end', 'space-between': 'space-between' };
        if (comp.d.align)
            el.style.alignItems = alignMap[comp.d.align] || comp.d.align;
        if (comp.d.justify)
            el.style.justifyContent = justifyMap[comp.d.justify] || comp.d.justify;
    }
    // Apply grid styles
...
// Auto-inject styles
injectWelcomeStyles();
//# sourceMappingURL=builder-welcome.js.map