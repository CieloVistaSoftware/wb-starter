...
(function injectStyles() {
    if (document.getElementById('quick-edit-styles'))
        return;
...
function saveQuickEdit() {
    ...
    try {
        ...
        // Update data
        if (mapping.header)
            c.d[mapping.header] = headerVal;
        if (mapping.main)
            c.d[mapping.main] = mainVal;
        if (mapping.footer)
            c.d[mapping.footer] = footerVal;
        ...
        if (wbEl) {
            if (mapping.header)
                wbEl.dataset[mapping.header] = headerVal;
            if (mapping.main)
                wbEl.dataset[mapping.main] = mainVal;
            if (mapping.footer)
                wbEl.dataset[mapping.footer] = footerVal;
            // Re-render via WB
            if (window.WB) {
        ...
        if (window.toast)
            window.toast('Error saving changes');
    }
}
// =============================================================================
// TEST URL
...
function testUrl() {
    ...
    if (!url) {
        if (window.toast)
            window.toast('Enter a URL first');
        return;
    }
    // Handle anchor links
    if (url.startsWith('#')) {
        if (window.toast)
            window.toast('Anchor link: ' + url);
        return;
    }
    // Open in new tab
...
//# sourceMappingURL=builder-quick-edit.js.map