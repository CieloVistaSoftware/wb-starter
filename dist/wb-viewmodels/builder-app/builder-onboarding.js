...
function showHint(hintKey) {
    ...
    if (hint.once && hintsShown.has(hint.id))
        return;
    // Add to queue
    hintQueue.push(hint);
    processHintQueue();
...
function processHintQueue() {
    if (hintTimeout || hintQueue.length === 0)
        return;
...
function showFirstTimeWelcome() {
    if (hintsShown.has('first-time-welcome'))
        return;
    const canvas = document.getElementById('canvas');
    const hasContent = canvas && canvas.querySelectorAll('.dropped').length > 0;
    if (hasContent)
        return; // Don't show if already has content
...
window.resetOnboardingHints = () => {
    ...
    if (window.toast)
        window.toast('Onboarding hints reset');
};
// =============================================================================
// STYLES
// =============================================================================
...
//# sourceMappingURL=builder-onboarding.js.map