...
export function updateElementId(oldId, newId) {
    ...
    if (!newId) {
        if (window.toast)
            window.toast('ID cannot be empty');
        return;
    }
    if (document.getElementById(newId) && newId !== oldId) {
        if (window.toast)
            window.toast('ID already exists: ' + newId);
        return;
    }
    const sanitized = newId.replace(/[^a-zA-Z0-9_-]/g, '-');
    if (sanitized !== newId) {
        if (window.toast)
            window.toast('ID sanitized to: ' + sanitized);
        newId = sanitized;
    }
    w.id = newId;
    ...
    if (window.saveHist)
        window.saveHist();
    if (window.toast)
        window.toast('ID updated to: ' + newId);
}
// Expose to window
...
//# sourceMappingURL=builder-selection.js.map