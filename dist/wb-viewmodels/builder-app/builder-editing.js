...
function enableTextEditing(el, { onSave, WB } = {}) {
    ...
    const handleBlur = (e) => {
        ...
        if (hrefInput && e.relatedTarget === hrefInput)
            return;
...
export function injectEditingStyles() {
    if (document.getElementById('wb-editing-styles'))
        return;
...
export function addResizeHandle(wrapper) {
    ...
    if (wrapper.querySelector('.wb-resize-handle'))
        return;
...
export function injectSnapGridStyles() {
    if (document.getElementById('wb-snap-grid-styles'))
        return;
    const style = document.createElement('style');
...
function getSnapTargets(canvas, excludeEl) {
    ...
    canvas.querySelectorAll('.dropped').forEach(el => {
        if (el === excludeEl || el.contains(excludeEl) || excludeEl.contains(el))
            return;
...
//# sourceMappingURL=builder-editing.js.map