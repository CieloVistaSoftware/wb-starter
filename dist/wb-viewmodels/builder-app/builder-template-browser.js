...
function render() {
    ...
    if (!container) {
        ...
        if (resizeHandle)
            resizeHandle.after(container);
        else if (collapsedLabel)
            collapsedLabel.after(container);
        else if (toggle)
...
async function useTemplate(id) {
    ...
    if (!template.components) {
        ...
        if (html) {
            ...
            if (window.toast)
                window.toast('Template file not found');
        }
        triggerOnboardingHint();
...
function findMatchingTemplate(sectionId, sectionTitle) {
    ...
    return templates.find(t => {
        ...
        return termsArray.some(term => tName.includes(term) || tId.includes(term) || tCat.includes(term));
    });
}
/**
...
function injectStyles() {
    if (document.getElementById('tbStyles'))
        return;
...
//# sourceMappingURL=builder-template-browser.js.map