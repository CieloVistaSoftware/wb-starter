...
function isPlaceholder(value, fieldType = 'text') {
    if (!value || value === '')
        return { isPlaceholder: false, isEmpty: true };
...
function getFieldType(fieldName) {
    if (['href', 'primaryHref', 'secondaryHref', 'ctaHref'].includes(fieldName))
        return 'href';
    if (['src', 'image', 'imageSrc', 'avatar', 'cover', 'video', 'audio'].includes(fieldName))
        return 'src';
    if (['url', 'shareUrl'].includes(fieldName))
        return 'url';
...
function injectStyles() {
    if (document.getElementById('incomplete-styles'))
        return;
...
function init() {
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        const isBadgeUpdate = mutations.every(m => {
            ...
            const addedAreBadges = addedNodes.length === 0 || addedNodes.every(n => n.nodeType === 1 && n.classList.contains('incomplete-badge'));
            const removedAreBadges = removedNodes.length === 0 || removedNodes.every(n => n.nodeType === 1 && n.classList.contains('incomplete-badge'));
...
// Exports
export { analyzeComponent, analyzeCanvas, updateBadges, showTemplateChecklist, showIssuesPanel, showAllIssues, updateFooterStatus, isPlaceholder, REQUIRED_FIELDS, PLACEHOLDER_PATTERNS };
export default {
    analyzeComponent,
    analyzeCanvas,
    updateBadges,
...
//# sourceMappingURL=builder-incomplete.js.map