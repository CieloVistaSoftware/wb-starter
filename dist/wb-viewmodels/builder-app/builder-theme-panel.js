...
window.setMode = (mode) => {
    ...
    updateCodePreview();
    if (window.toast)
        window.toast(`Mode: ${mode === 'auto' ? 'System' : mode.charAt(0).toUpperCase() + mode.slice(1)}`);
};
/**
 * Set theme
 */
window.setTheme = (themeId) => {
    ...
    if (window.toast)
        window.toast(`Theme: ${theme?.name || themeId}`);
};
/**
...
function injectThemePanelStyles() {
    if (document.getElementById('themePanelStyles'))
        return;
...
//# sourceMappingURL=builder-theme-panel.js.map