...
export function addTemplateHTML(html, templateMeta = {}) {
    ...
    updateBadges();
    if (window.toast)
        window.toast('Added ' + (templateMeta.name || 'Template'));
}
/**
...
export function addTemplate(template) {
    ...
    if (window.toast)
        window.toast(`Added "${template.name}" template`);
    if (window.renderTree)
        window.renderTree();
    if (window.saveHist)
        window.saveHist();
...
<body>
  ...
  `;
    ...
    if (win) {
        ...
        if (window.toast)
            window.toast('Please allow popups to preview');
    }
}
/**
...
//# sourceMappingURL=builder-template-add.js.map