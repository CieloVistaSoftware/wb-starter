    ...
    container.querySelectorAll('.comp-item').forEach(item => {
        ...
        item.addEventListener('click', () => {
            ...
            if (window.add) {
                ...
                if (clickComp.b)
                    trackComponentUsage(clickComp.b);
            }
        });
    });
}
...
function handleSearch(query) {
    ...
    if (!query.trim()) {
        ...
        if (recentSection && recentComponents.length > 0)
...
function injectSidebarStyles() {
    if (document.getElementById('sidebarSearchStyles'))
        return;
...
//# sourceMappingURL=builder-sidebar.js.map