...
(function injectTreeStyles() {
    if (document.getElementById('tree-compact-styles'))
        return;
...
function getElementDisplayInfo(el) {
    ...
    if (tag === 'img') {
        ...
        if (paraText)
            name = `${tag}: "${paraText}${paraText.length >= 30 ? '…' : ''}"`;
    }
    else if (tag === 'button') {
...
function hasVisibleChildren(el) {
    ...
    const dominated = Array.from(el.children).filter(c => !['SCRIPT', 'STYLE', 'BR', 'HR'].includes(c.tagName));
...
function findMeaningfulContent(root) {
    ...
    function walk(el) {
        ...
        for (const child of el.children) {
            ...
            if (['SCRIPT', 'STYLE', 'NOSCRIPT', 'TEMPLATE', 'BR'].includes(child.tagName))
                continue;
...
function renderDOMChild(el, level = 1, maxDepth = 6, parentPath = 'root', showChildren = true) {
    if (level > maxDepth)
        return ''; // Prevent infinite depth
    const info = getElementDisplayInfo(el);
    const indentStyle = `width: ${level * 12}px;`;
    // For flattened content display, don't show expand toggle
    const children = showChildren ? Array.from(el.children).filter(c => !['SCRIPT', 'STYLE', 'BR', 'NOSCRIPT', 'TEMPLATE'].includes(c.tagName)) : [];
...
window.inspectDOMElement = (elKey) => {
    ...
    navigator.clipboard.writeText(text).then(() => {
        if (window.toast)
            window.toast('Element debug info copied!');
    }).catch(err => {
...
function initTree() {
    ...
    document.addEventListener('wb:canvas:section:toggled', (e) => {
        ...
        if (canvasToggledSection) {
            ...
            if (expanded)
                activeSection = canvasToggledSection;
            renderTree();
        }
    });
    ...
    document.addEventListener('wb:template:section:toggled', (e) => {
        ...
        if (templateToggledSection) {
            ...
            if (expanded)
                activeSection = templateToggledSection;
            renderTree();
        }
    });
...
//# sourceMappingURL=builder-tree.js.map