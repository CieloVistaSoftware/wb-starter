...
export function notes(element, options = {}) {
  ...
  `;
        ...
        try {
            ...
            if (local) {
                ...
                if (config.restoreState) {
                    if (loadedData.position)
                        setPosition(loadedData.position);
                    if (loadedData.width)
                        drawer.style.width = loadedData.width;
                    if (loadedData.modalPos)
                        modalPos = loadedData.modalPos;
                    if (loadedData.modalSize)
                        modalSize = loadedData.modalSize;
    ...
    const viewSavedNotes = () => {
        ...
        viewer.onclick = (e) => { if (e.target === viewer)
            closeViewer(); };
    ...
    // MODAL DRAG
    const onDragMove = (e) => {
        if (!isDragging || currentPosition !== 'modal')
            return;
    ...
    const onDragStart = (e) => {
        if (e.target.closest('button') || currentPosition !== 'modal')
            return;
        isDragging = true;
        dragStart.x = e.clientX;
        dragStart.y = e.clientY;
    ...
    const onModalResizeMove = (e) => {
        if (!isResizing || currentPosition !== 'modal')
            return;
    ...
    element.addEventListener('click', (e) => {
        ...
        if (action === 'collapse-left')
            collapseToSide('left');
        else if (action === 'collapse-right')
            collapseToSide('right');
        else if (action === 'copy')
            copyToClipboard();
        else if (action === 'view')
            viewSavedNotes();
