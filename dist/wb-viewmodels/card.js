...
const parseBoolean = (val) => {
    ...
    if (val === 'false')
        return false;
    if (val === '')
        return true; // Handle boolean attributes (e.g. data-clickable="")
    return val;
};
...
export function cardBase(element, options = {}) {
            ...
            try {
                ...
                try {
                    if (config.dataContext)
                        data = JSON.parse(config.dataContext);
                }
                catch (err) {
    ...
    return {
        ...
        buildStructure: (options = {}) => {
            const { headerContent = '', mainContent = '', footerContent = '', showHeader = true, showMain = true, showFooter = true } = options;
        ...
        // Cleanup function
        cleanup: () => {
            element.classList.remove('wb-card', `wb-card--${config.behavior.replace('card', '')}`, `wb-card--${config.variant}`, `wb-card--${config.size}`, 'wb-card--hoverable', 'wb-card--elevated', 'wb-card--clickable', 'wb-card--active');
            if (config.hoverable) {
...
export function cardhero(element, options = {}) {
    ...
    ['pretitle', 'title', 'subtitle'].forEach(slotName => {
        ...
        if (!slotEl)
            slotEl = element.querySelector(`[data-slot="${slotName}"]`);
        if (slotEl) {
    ...
    if (slots.title) {
        ...
        if (!slots.title.style.fontSize)
            slots.title.style.fontSize = '2.5rem';
        if (!slots.title.style.marginBottom)
            slots.title.style.margin = '0';
    ...
    if (base.config.cta || base.config.ctaSecondary) {
        ...
        if (config.xalign === 'left')
            ctaGroup.style.justifyContent = 'flex-start';
        if (config.xalign === 'right')
            ctaGroup.style.justifyContent = 'flex-end';
        if (base.config.cta) {
...
export function cardfile(element, options = {}) {
    ...
    const meta = [];
    if (config.size)
        meta.push(config.size);
    if (config.date)
        meta.push(config.date);
    if (meta.length) {
...
export function cardexpandable(element, options = {}) {
    ...
    if (config.expanded)
        icon.classList.add('wb-card__expand-icon--expanded');
    icon.textContent = '▼';
    ...
    if (isExpanded)
        element.classList.add('wb-card--expanded');
    const toggle = () => {
...
export function cardminimizable(element, options = {}) {
    ...
    if (isMinimized)
        element.classList.add('wb-card--minimized');
    const toggle = () => {
    ...
    element.wbCardMinimizable = {
        toggle,
        minimize: () => { if (!isMinimized)
            toggle(); },
        expand: () => { if (isMinimized)
            toggle(); },
