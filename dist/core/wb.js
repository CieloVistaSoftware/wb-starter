...
function getAutoInjectBehavior(element) {
    ...
    if (element.hasAttribute(`${prefix}-${candidate}`))
        return null;
    if (element.hasAttribute(candidate) && !RESERVED_ATTRIBUTES.has(candidate))
        return null;
    if (element.hasAttribute(`data-wb-${candidate}`))
        return null;
    return candidate;
}
...
const WB = {
    ...
    processSchema(element, schemaName = null) {
        if (schemaProcessed.has(element))
            return;
    ...
    async scan(root = document.body) {
        ...
        root.querySelectorAll('[data-wb]').forEach(element => {
            if (!(element instanceof HTMLElement))
                return; // Ensure element is an HTMLElement
     ...
     */
    async init(options = {}) {
        const { scan: shouldScan = true, observe: shouldObserve = true, theme = null, debug = false, autoInject = false, // Default to false unless specified
