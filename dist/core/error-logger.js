...
export async function logError(message, details = {}) {
  ...
  `;
    ...
    let detailsHtml = '';
    if (details.file)
        detailsHtml += `<div style="color:#888;font-size:11px;">📁 ${details.file}:${details.line || '?'}</div>`;
    if (details.stack)
        detailsHtml += `<div style="color:#666;font-size:10px;margin-top:4px;max-height:60px;overflow:auto;">${escapeHtml(details.stack)}</div>`;
    item.innerHTML = `
