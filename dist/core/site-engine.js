...
export default class WBSite {
    ...
    updateFavicon() {
        if (!this.config.branding.browserTabIcon)
            return;
    ...
    renderFooter() {
        ...
        const socialLinks = footerSettings.displaySocialMediaLinks ? socialMediaLinks.map(s => `<a href="${s.profileUrl}" class="footer__social-link" target="_blank" title="${s.platform}" id="footerSocialLink-${s.platform}">${s.icon}</a>`).join('') : '';
        const footerLinks = additionalFooterLinks?.map(l => `<a href="?page=${l.pageToLoad}" class="footer__link" id="footerLink-${l.pageToLoad}">${l.linkText}</a>`).join(' · ') || '';
        return `
...
//# sourceMappingURL=site-engine.js.map