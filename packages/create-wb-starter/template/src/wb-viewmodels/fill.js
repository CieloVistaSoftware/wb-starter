...
export function fill(element, options = {}) {
  ...
  if (FLEX.has(parentDisplay)) {
    ...
    added.push('x-fill--grid');
  } else {
    // Block formatting context. An inline element needs a box before width
    // applies at all -- .x-fill--block sets display:block for exactly that.
    added.push('x-fill--block');
  }

  if (config.ignoreMaxWidth) added.push('x-fill--ignore-max');
