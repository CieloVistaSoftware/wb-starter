...
test.describe('x-grid Behavior', () => {
  ...
  test('element is visible after scan', async ({ page }) => {
    const html = '<div x-grid>Test content</div>';
    await injectAndScan(page, html);
    
    const el = page.locator('#test-container x-grid, #test-container [x-wb-grid]').first();
