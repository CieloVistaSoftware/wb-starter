/**
 * x-cluster Behavior Tests
 * Auto-generated baseline — verifies render + no console errors
 * Source: src/wb-viewmodels/x-cluster.js
 */
...
test.describe('x-cluster Behavior', () => {
  ...
  test('renders without errors', async ({ page }) => {
    ...
    const setupHtml = [
      '<div x-cluster>Test content</div>'
    ];
  ...
  test('element is visible after scan', async ({ page }) => {
    const html = '<div x-cluster>Test content</div>';
    await injectAndScan(page, html);
    
    const el = page.locator('#test-container x-cluster, #test-container [x-wb-cluster]').first();
