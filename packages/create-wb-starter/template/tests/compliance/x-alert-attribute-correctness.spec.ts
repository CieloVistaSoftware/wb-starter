...
/**
 * <div x-alert>'s real behavior (alert() in src/wb-viewmodels/feedback.js) reads
...
test('<div x-alert> never uses type= — only variant= is read (attribute audit)', () => {
  const files: string[] = [];
  walk(ROOT, files);

  const offenders: string[] = [];
  const WB_ALERT_TYPE = /<div x-alert\b[^>]*\btype=/;
  ...
  expect(
    offenders,
    `<div x-alert type="..."> found — alert() only reads 'variant', type= is silently ignored (every instance falls back to the 'info' default):\n${offenders.join('\n')}`
  ).toEqual([]);
});
