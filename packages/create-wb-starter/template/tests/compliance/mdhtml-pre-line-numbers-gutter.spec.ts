 ...
 *
 * Root cause: mdhtml.css's `.x-mdhtml pre { padding: 1rem; }` has higher
 ...
 * <pre> that happens to render inside a <div x-mdhtml> container (every code
 * block viewed via the doc-viewer).
 *
 * Fixed with `.x-mdhtml pre:not(.x-pre)` — once pre.js has enhanced a
