 ...
 *        <summary> element and no .x-details__content wrapper, so the 1rem
...
test.describe('#689 — native <details> gets a summary element and a content wrapper', () => {
  ...
  test('summary and content share one inset, and the image sits on it', async ({ page }) => {
    ...
    const geo = await page.evaluate(() => {
      ...
      const content = det.querySelector('.x-details__content') as HTMLElement;
    ...
    expect(geo.hasContent, 'children must be wrapped in .x-details__content').toBe(true);
