/**
 * <div x-cardlink>'s click handler called e.preventDefault() then
...
test.describe('<div x-cardlink> renders a real stretched <a> (not JS window.open)', () => {
  test('a real anchor with the right href/target/rel exists inside the card', async ({ page }) => {
    await setup(page, '<div x-cardlink id="cl1" title="Example" href="/demos/site/forms.html" target="_blank"></div>');
  ...
  test('the anchor is stretched to cover the whole card (position:absolute, inset:0)', async ({ page }) => {
    await setup(page, '<div x-cardlink id="cl2" title="Example" href="/demos/site/forms.html" target="_blank"></div>');
  ...
  test('clicking the card triggers real navigation (a new tab opens, not window.open())', async ({ page, context }) => {
    await setup(page, '<div x-cardlink id="cl3" title="Example" href="/demos/site/forms.html" target="_blank"></div>');
