...
async function scrollAllDemosIntoView(page: Page) {
  ...
  // gets triggered.
  await page.locator('x-demo').first().waitFor({ state: 'attached', timeout: 20000 });
...
test.describe('x-demo source is vertical + never horizontally scrolls (#254, #248)', () => {
  test('§6 — no x-demo source panel horizontally scrolls', async ({ page }) => {
    ...
    const offenders = await page.$$eval('x-demo pre', (els) =>
      els
        .map((el, i) => ({
          i,
    ...
    expect(offenders, `x-demo source panels that scroll or are center-aligned:\n${JSON.stringify(offenders, null, 2)}`).toEqual([]);
  });
