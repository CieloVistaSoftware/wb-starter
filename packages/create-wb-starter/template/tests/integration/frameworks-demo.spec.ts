...
test.describe('frameworks demo: x-demo / build-step exception (§25, #324, #460)', () => {
  ...
  test('Svelte section compiles client-side and renders live, interactively, with its real source shown (#460)', async ({ page }) => {
    ...
    await expect(button, 'svelte/compiler (vendored under src/lib/frameworks/svelte) must compile and mount a real behavior').toBeVisible({ timeout: 15000 });
  ...
  test('SolidJS section compiles client-side and renders live, interactively, with its real source shown (#460)', async ({ page }) => {
    ...
    await expect(button, 'babel-plugin-jsx-dom-expressions (vendored, run via vendored @babel/standalone) must compile and mount a real behavior').toBeVisible({ timeout: 15000 });
