 ...
 * same-origin localStorage key 'x-theme'. The doc-viewer is a standalone page,
 ...
 */
test('doc-viewer applies the persisted x-theme on load', async ({ page }) => {
  // Seed the persisted theme on the app origin before the doc-viewer loads.
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  await page.evaluate(() => localStorage.setItem('x-theme', 'ocean'));

  await page.goto('/public/doc-viewer.html?file=docs/index.md', { waitUntil: 'domcontentloaded' });

  const applied = await page.evaluate(() => document.documentElement.getAttribute('data-theme'));
  expect(applied, 'doc-viewer did not apply the persisted x-theme').toBe('ocean');
});
