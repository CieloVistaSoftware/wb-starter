 ...
 * written into the <wb-dialog> host's own innerHTML. dialog.schema.json's
 ...
 * chrome into a live <wb-dialog> host. wb-dialog was added to
 ...
 */
test.describe('wb-dialog is excluded from schema $view building (#387)', () => {
  test('/?page=forms: schema does not write div/header/h2/main/footer chrome into a live <wb-dialog> host', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      container.id = 'wb-dialog-387-test';
      container.innerHTML = '<wb-dialog id="dlg-387" title="Test Dialog">Open the dialog</wb-dialog>';
      document.body.appendChild(container);
    });
    await page.evaluate(() =>
      (window as any).WB.scan(document.getElementById('wb-dialog-387-test'), { eager: true })
    );
    ...
    await expect(host).toHaveClass(/wb-dialog-trigger/);
    await expect(host).toHaveClass(/wb-modal/);
    ...
    // children of the live <wb-dialog>.
    const builtSchemaChrome = await host.evaluate((el) =>
      !!el.querySelector(':scope > header, :scope > h2, :scope > main, :scope > footer')
    );
    expect(builtSchemaChrome, 'schema must not build $view chrome into a live <wb-dialog> host').toBe(false);
    ...
    await expect(dlg.locator('.wb-dialog__title')).toHaveText('Test Dialog');
  });
});
