...
/**
 * #489 (split off #322): <wb-switch> never rendered at all on wb-lazy.js
 ...
 * <wb-switch> shared a page, a schema-vs-behavior race meant only the FIRST
 ...
 * so asserting on `.wb-switch__track` alone (as switch-size-variant-no-op.spec.ts
...
test.describe('wb-switch schema builds on wb-lazy.js pages (#489)', () => {
  test('a single <wb-switch> gets x-schema and a real schema-built checkbox input', async ({ page }) => {
    await injectAndScan(page, '<wb-switch id="sw1" label="Notifications"></wb-switch>');
    ...
    await expect(host.locator('.wb-switch__track')).toHaveCount(1);
    await expect(host.locator('.wb-switch__thumb')).toHaveCount(1);
    ...
    await expect(host.locator('.wb-switch__label-end')).toHaveText('Notifications');
  });

  test('multiple <wb-switch> instances on one page all get schema-built (no first-wins race)', async ({ page }) => {
    const ids = await injectAndScan(
      page,
      `
      <wb-switch id="sw-a" label="A"></wb-switch>
      <wb-switch id="sw-b" label="B" size="lg"></wb-switch>
      <wb-switch id="sw-c" label="C" variant="primary"></wb-switch>
      <wb-switch id="sw-d" label="D" checked></wb-switch>
      <wb-switch id="sw-e" label="E" disabled></wb-switch>
      `
    );
    ...
    await expect(page.locator('#sw-b')).toHaveClass(/wb-switch--lg/);
    await expect(page.locator('#sw-c')).toHaveClass(/wb-switch--primary/);
  });
});
