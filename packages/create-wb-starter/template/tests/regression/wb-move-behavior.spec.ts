 ...
 *    tag-map.js's elementMap/extensionMap already mapped <wb-move> and
 ...
 */
test.describe('wb-move behavior (#344)', () => {
  test('<wb-move> resolves without "Unknown behavior" and gets the baseClass', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', (msg) => {
    ...
    await expect(host).toHaveClass(/(^|\s)wb-move(\s|$)/, { timeout: 10000 });
  ...
  test('moveright/moveleft swap adjacent grid items without throwing (#344)', async ({ page }) => {
    ...
    await page.goto('/');
    await page.setContent(`
      <wb-move id="mv" style="display:grid;grid-template-columns:1fr 1fr;">
