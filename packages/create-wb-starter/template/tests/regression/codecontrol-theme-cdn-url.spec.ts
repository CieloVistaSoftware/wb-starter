 ...
 * immediately on <wb-codecontrol> init (not just on user selection), so
 * this broke syntax highlighting for the DEFAULT theme ('atom-one-dark')
 * on every page load that includes a <wb-codecontrol>, plus any of the
 ...
 * a <wb-codecontrol> instance, rather than synthetic injection.
 */
import { test, expect } from '@playwright/test';

const PAGE_URL = '/demos/site/content.html';

test.describe('wb-codecontrol theme URLs must never point at a dev-only path', () => {
  ...
  test('selecting the local wb-grayscale-dark theme still resolves to its real local file, not a CDN URL', async ({ page }) => {
    await page.goto(PAGE_URL);
    await page.waitForFunction(() => {
      const el = document.querySelector('wb-codecontrol') as any;
      return !!(el && el.wbCodeControl);
    ...
    await page.evaluate(() => {
      const el = document.querySelector('wb-codecontrol') as any;
      el.wbCodeControl.setTheme('wb-grayscale-dark');
    });
    await page.waitForTimeout(200);

    const href = await page.locator('link[data-highlight-theme]').getAttribute('href');
    expect(href, 'a local WB theme must not be misrouted to cdnjs').not.toContain('cdnjs.cloudflare.com');
    expect(href, 'a local WB theme must resolve to its real local file').toContain('/src/styles/code-themes/wb-grayscale-dark.css');
