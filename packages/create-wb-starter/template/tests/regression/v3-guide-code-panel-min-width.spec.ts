...
/**
 * Live-reported: docs/V3-GUIDE.md's bare <wb-spinner>/<wb-progress> examples
 ...
 * `wb-demo:has(> .wb-demo__grid--cols-1 > :only-child) { width: var(--wb-
 * demo-shrink-width, fit-content) }`) sizes the WHOLE demo -- code panel
 * included, since .wb-demo__code is width:100% of its wb-demo parent -- to
 ...
 *
 * Fix: .wb-demo__code (and its wb-demo parent, so the child's min-width
 ...
 */
test.describe('wb-demo code panels never collapse to unreadable vertical strips', () => {
  test('V3-GUIDE.md: no wb-demo widget renders narrower than the readable-code floor', async ({ page }) => {
    ...
    expect(count, 'expected the guide to have multiple live wb-demo examples').toBeGreaterThan(1);
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(
        box.width,
        `wb-demo[${i}] rendered only ${Math.round(box.width)}px wide -- collapsed to an unreadable strip.`
      ).toBeGreaterThan(150);
    }
  });
  ...
  test('the Spinner example specifically is readable, not a single-character-wide strip', async ({ page }) => {
    ...
    const demo = label.locator('xpath=following-sibling::wb-demo[1]');
    const box = await demo.boundingBox();
    expect(box, 'the Spinner wb-demo must have a measurable box').not.toBeNull();
    expect(box!.width).toBeGreaterThan(150);

    const codeText = await demo.locator('.wb-demo__code').innerText();
    expect(codeText, 'code panel text should read as normal wrapped lines, not one character per line').toContain('wb-spinner');
  });
});
