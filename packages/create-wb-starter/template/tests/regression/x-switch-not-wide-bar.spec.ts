...
/**
 * <div x-switch label="Test"> rendered a long, wide, empty rounded bar BEFORE
...
test.describe('x-switch does not render a stray wide bar', () => {
  ...
  test('the internal checkbox stays visually hidden, only the small track/thumb show', async ({ page }) => {
    const el = await setupTestContainer(page, '<div x-switch label="Test"></div>');
    ...
    const track = el.locator('.x-switch__track');
