 ...
 *      layout.css also defaulted every <div x-drawer> to `visibility: hidden`
 *      (a rule meant for DEFINITION-mode drawers, not self-triggering ones).
 *   2. tag-map.js separately maps <div x-drawer> to the legacy `drawer()`
...
test.describe('<div x-drawer> trigger renders and opens correctly (not a broken box)', () => {
  ...
  test('exactly one panel+backdrop pair exists per trigger (no duplicate/competing overlay)', async ({ page }) => {
    ...
    await expect(page.locator('.x-drawer__panel')).toHaveCount(2);
    await expect(page.locator('.x-drawer__backdrop')).toHaveCount(2);
    ...
    const panelsInsideTriggers = await page.locator('x-drawer .x-drawer__panel').count();
  ...
  test('clicking the close button closes the panel', async ({ page }) => {
    ...
    await page.locator('.x-drawer__close').first().click();
