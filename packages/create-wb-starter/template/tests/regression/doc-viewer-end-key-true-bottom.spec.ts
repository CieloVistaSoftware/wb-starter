 ...
 * ?file=docs/components/cards/card.md, which embeds 10 <wb-demo> blocks)
 ...
 * docs/manifest.json fetch before appending .wb-demo__grid + doc-link
 ...
 * <wb-demo> to build immediately by calling the same demo() function
 * wb-demo.js's own IntersectionObserver callback would eventually call
...
test.describe('doc-viewer.html End key reaches the true page bottom (#466)', () => {
  test('End on a doc with lazy <wb-demo> blocks lands within a few px of true bottom', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 855 });

    // 'load' (not 'networkidle') + a short pause deliberately catches the
    // page BEFORE all lazy <wb-demo> blocks have settled — matching how a
    ...
    const demoCount = await page.locator('wb-demo').count();
    expect(demoCount, 'fixture doc must actually embed <wb-demo> blocks for this test to be meaningful').toBeGreaterThan(5);
  ...
  test('End on a plain doc with no <wb-demo> still scrolls to the bottom (no regression)', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 855 });
