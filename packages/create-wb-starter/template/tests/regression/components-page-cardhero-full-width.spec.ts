...
/**
 * pages/behaviors.html: the standalone <div x-cardhero> under "Overlay & Hero
 ...
 * banner-style behavior meant to fill its container even alone, so
 ...
 * full-width>`) -- pages/behaviors.html:63 is the one instance that never
 * got the attribute when authored.
 */

test('?page=behaviors: standalone <div x-cardhero> demo spans full width, not a narrow sliver', async ({ page }) => {
