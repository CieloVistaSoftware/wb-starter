 ...
 * matching `wb-select--{{size}}` / `wb-select--{{variant}}` classes -- but
 * ONLY to the host <wb-select> wrapper (select.js lines 101-102). The
 ...
 * `.wb-select--*` rules (lines 82-123) all target the host tag/class, and
 ...
 * ignoring variant entirely. Confirmed by John pasting ~23 wb-select combos
...
async function inject(page: import('@playwright/test').Page, html: string) {
  ...
  // Wait for the real <select class="wb-select__field"> buildWbSelect()
  // creates, proving the behavior actually ran (not just that the host
  // element exists).
  await page.waitForFunction(
    (elementIds: string[]) => elementIds.every((id) => document.getElementById(id)?.querySelector('select.wb-select__field')),
    ids,
    { timeout: 5000 }
  );
}

test.describe('wb-select size/variant classes actually differ from default', () => {
  test('xs/sm/md/lg render distinct rendered <select> padding', async ({ page }) => {
    await inject(page, `
      <wb-select id="sel-xs" size="xs" options='[{"value":"1","label":"One"}]'></wb-select>
      <wb-select id="sel-sm" size="sm" options='[{"value":"1","label":"One"}]'></wb-select>
      <wb-select id="sel-md" size="md" options='[{"value":"1","label":"One"}]'></wb-select>
      <wb-select id="sel-lg" size="lg" options='[{"value":"1","label":"One"}]'></wb-select>
    `);
    ...
    const paddings = await Promise.all(
      ['#sel-xs', '#sel-sm', '#sel-md', '#sel-lg'].map((sel) =>
        page.locator(`${sel} select.wb-select__field`).evaluate((el) => getComputedStyle(el).padding)
      )
    );
  ...
  test('default/success/error render three distinct rendered <select> border colors', async ({ page }) => {
    await inject(page, `
      <wb-select id="sel-default" variant="default" options='[{"value":"1","label":"One"}]'></wb-select>
      <wb-select id="sel-success" variant="success" options='[{"value":"1","label":"One"}]'></wb-select>
      <wb-select id="sel-error" variant="error" options='[{"value":"1","label":"One"}]'></wb-select>
    `);
    const colors = await Promise.all(
      ['#sel-default', '#sel-success', '#sel-error'].map((sel) =>
        page.locator(`${sel} select.wb-select__field`).evaluate((el) => getComputedStyle(el).borderColor)
      )
    );
