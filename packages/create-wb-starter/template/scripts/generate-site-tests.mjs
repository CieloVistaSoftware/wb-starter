 ...
 * =======================
 * Reads test.site from each behavior schema and generates
 ...
 *   - For each behavior: renders, has base class, text visible, classes applied
 */
import { readFileSync, writeFileSync, mkdirSync, existsSync } from 'fs';
import { resolve, join } from 'path';

const MODELS = resolve('src/wb-models');
const SITE_CONFIG = resolve('src/wb-models/pages/x-behavior-library.site.json');
...
for (const page of site.pages) {
  ...
  for (const compName of page.behaviors) {
    ...
    try {
      ...
      pageSchemas[page.id].behaviors.push({
        name: compName,
        ...siteTest,
...
for (const [pageId, pageData] of Object.entries(pageSchemas)) {
  const comps = pageData.behaviors;
  ...
  lines.push(` * Behaviors: ${comps.length}`);
  ...
  for (const comp of comps) {
    ...
    // Test 1: Behavior exists on page
  ...
  console.log(`  ✅ ${pageId}.spec.ts — ${comps.length} behaviors, ~${testCount} tests`);
