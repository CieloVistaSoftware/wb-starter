...
- Fixed `x-copy` reading the wrong attribute name (schema says `text`, code read `copy-text`) and `x-darkmode`'s click-to-toggle only ever attaching to literal `<button>` tags (#429).
...
- **#482** — `x-cardvideo` has no `aspect-ratio` unlike `x-cardimage` (inconsistent box height on load failure).
