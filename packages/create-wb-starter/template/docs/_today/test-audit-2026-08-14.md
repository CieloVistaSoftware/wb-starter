...
| 4 | Events work | `tests/compliance/x-demo-events-fire.spec.ts` (new — **no prior coverage of any kind**) | none |

Every new/extended test discovers its own file list by scanning the repo (glob +
grep for the relevant marker — `<div x-demo`, `events=`, etc.), not a hand-maintained
...
| 2. Examples work | 19 (every demos/**/*.html + pages/**/*.html that renders `<div x-demo>`) | 10 | **8** (62 individual broken/empty demo instances) | 1 |
...
| 4. Events fire | 4 files / 7 `x-demo[events]` instances (the only ones in the repo today) | 4 | **0** | 0 |
...
npx playwright test tests/compliance/x-demo-events-fire.spec.ts --workers=2
```
...
- `docs/behaviors/x-audio.md` → `../../src/wb-models/x-audio.schema.json`
- `docs/behaviors/x-card.md` → `../../src/wb-models/x-card.schema.json`
- `docs/behaviors/x-column.md` → `../../src/wb-viewmodels/x-column.js`
- `docs/behaviors/x-row.md` → `../../src/wb-viewmodels/x-row.js`
- `docs/behaviors/x-search.md` → `../../src/wb-viewmodels/x-search.js`
- `docs/behaviors/x-stack.md` → `../../src/wb-viewmodels/x-stack.js`
- `docs/behaviors/x-themecontrol.md` → `../../demos/semantics-theme.html`
...
- `docs/components/tabs.md` → 4 dead links (`tabs.schema.json`, `x-tabs.js`, `tabs.js`, `tabs.css`)
- `docs/card.md` → `../tests/behaviors/ui/card.spec.ts`
- `docs/search.md` → `../src/wb-viewmodels/x-search.js`, `../demo-search.html`
...
every `<div x-demo>`'s `.x-demo__grid` must render at least one real, non-zero-size
...
`<div x-demo>` through the doc-viewer (99 docs, `docs/_today/**` excluded as session
scratch) PLUS every plain demo/page `.html` file that renders `<div x-demo>` directly (19
...
wrap-exclusion for `.x-demo__code` panels holds up site-wide, not just on the one file
...
**Test:** `tests/compliance/x-demo-events-fire.spec.ts` — **the one dimension with
zero prior coverage of any kind** before this pass. For every `<div x-demo events="...">`
...
`[role=tab]` → `<tr>` → first grid child) and asserts `.x-demo__events-log-entry`
...
- **Dimension 3:** the original single-file version of this test used a fixed
  `waitForFunction` that assumed every `<div x-demo>` on the page has at least one code
  ...
  can't block the whole file's audit), and some pages legitimately have `<div x-demo>`
