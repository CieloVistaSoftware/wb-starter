# Audit: `.x-demo__code` panels on `docs/standards/V3-STANDARDS.md` via doc-viewer
...
— every `.x-demo__code` panel on the page (the `<pre x-behavior="pre">` block `<div x-demo>`
...
The doc renders **11** `.x-demo__code` panels total (10 authored `<div x-demo>` blocks in
...
/* §6: pre.js (x-behavior="pre") defaults to editor-style no-wrap
   ...
   call for x-demo's live code samples elsewhere, but wrong for prose docs
...
pre.js-enhanced block), is not scoped to exclude `<div x-demo>`-generated panels — and
`docs/standards/DEMOS-AND-DOCS-STANDARDS.md` §6 explicitly carves those out as the
opposite case:

> **Carve-out for `<div x-demo>`-generated code panels specifically (#390):** these use
...
Because `.x-demo__code` panels ARE `.x-pre` elements, and doc-viewer.html renders
...
vs. one class) **on every `<div x-demo>` viewed through the doc-viewer** — silently
...
fixed for this exact rule family (`x-mdhtml pre:not(.x-pre)`, see
...
below it (added for #195) doesn't scope out `<div x-demo>` panels the same way.

Once wrapping is active, **check (b)'s violations are a direct downstream effect**: any
`<div x-demo>` source line long enough to wrap (an attribute value, mainly) spans multiple
...
Every `.x-demo__code` panel on the page has computed `white-space: pre-wrap` instead of
the required `pre`:

| Demo | Doc section | First line of source | Computed `white-space` |
|---|---|---|---|
| 0 | Component vs. Behavior → Component | `<article` | `pre-wrap` |
| 1 | Component vs. Behavior → Component | `<dialog title="Confirm action">` | `pre-wrap` |
...
**demo 1 (`<dialog title="Confirm action">`, Component vs. Behavior → Component)** — 4
gaps irregular:
...
## Incidental finding: an 11th panel not authored as a literal `<div x-demo>`

The doc's raw Markdown contains exactly **10** `<div x-demo>` blocks (confirmed via
`grep -c "<div x-demo" docs/standards/V3-STANDARDS.md`), but the rendered page shows 11
`.x-demo__code` panels. The 11th (demo index 10, "Migration from Legacy Syntax" section)
...
— gets converted into a real, live `<div x-demo>` because it contains a genuine `<article>`
...
awareness only; it's a `.x-demo__code` panel like any other so it's included in all three
...
- **All 11 `<div x-demo>` blocks were forced into view before measuring.**
  `src/wb-viewmodels/x-demo.js` only builds the first `EAGER_BUILD_COUNT` (5) blocks
  ...
  now calls `scrollIntoViewIfNeeded()` on every `<div x-demo>` before reading anything.
...
- **Why `.x-demo__code` panels can never legitimately contain a blank line.**
  ...
  `x-demo__events-code` panel, built by `buildEventListenerCode()`, is the one
  `.x-demo__code`-classed panel type that CAN contain a real blank line — a `\n\n`
...
1. **(c), root cause — mdhtml.css's `.x-pre` wrap override needs to exclude
   `<div x-demo>`-generated panels.** This single fix should also eliminate all 14 of the
   (b) violations, since they're downstream of wrapping. Scope the `x-mdhtml .x-pre` /
   `.x-mdhtml .x-pre` rule (lines 159-172) to exclude `.x-demo__code` specifically —
   mirroring how the padding rule just above it already excludes enhanced blocks via
   `:not(.x-pre)`, except here the exclusion needs to be more targeted (only x-demo
...
- **Tracking issue filed for the fix:** #583 (`doc-viewer: all x-demo code panels wrap
