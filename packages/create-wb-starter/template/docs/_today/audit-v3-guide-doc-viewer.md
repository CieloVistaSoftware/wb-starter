...
`x-demo:has(> .x-demo__grid--cols-1 > :only-child)`) sizes the *whole* demo — code
panel included, since `.x-demo__code` is `width:100%` of its `x-demo` parent — to the
...
**Fix:** `.x-demo__code` (and its `x-demo` parent, so a child's min-width isn't just
...
(`:has()` + descendant class) outranks the existing `x-demo[data-code-width="50vw"]`
...
**Test:** `tests/regression/v3-guide-code-panel-min-width.spec.ts` (2 cases — no x-demo
...
- `edb96d9` — doc examples auto-render as live `x-demo` widgets (the feature itself)
