...
```html
<button variant="primary" size="lg" class="x-button x-button--lg x-button--primary">
...
|-------|--------------|-------------|
| `.x-button` | Always, once enhanced | Base attribute-driven styling |
| `.x-button--{xs,sm,md,lg,xl}` | `size` set | Padding/font-size scale |
| `.x-button--{primary,secondary,success,danger,warning,info,ghost,link}` | `variant` set | Color scheme |
| `.x-button__icon` | `icon` set | Icon wrapper |
| `.x-button__spinner` | `loading` | Spinning loading glyph |
