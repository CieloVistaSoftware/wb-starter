...
|----------|------|---------|-------------|
| `label` | string | `""` | Label text shown above the field (`<div x-input>` form) |
...
<div x-demo>
<div x-input label="Full name" placeholder="Jane Doe"></div>
</div>

### Input Types

```html
<div x-input label="Email" input-type="email" placeholder="you@example.com"></div>
<div x-input label="Password" input-type="password" placeholder="••••••••"></div>
```

### Helper and Error Text

```html
<div x-input label="Username" helper="Must be at least 3 characters"></div>
<div x-input label="Username" error="This field is required" variant="error"></div>
```

### With Icon and Clearable

```html
<div x-input label="Search" icon="🔍" placeholder="Search…" clearable></div>
```

### Sizes

```html
<div x-input label="Small" size="sm"></div>
<div x-input label="Large" size="lg"></div>
```
...
<div x-input>
  ...
  <div class="x-input__wrapper">
    <span>🔍</span>
    <input type="text" class="x-input__field" placeholder="Search…">
    <button type="button">✕</button>
  </div>
  <span class="x-input__helper">Press Enter to search</span>
</div>
```
...
|-------|--------------|-------------|
| `.x-input__wrapper` | Always | Structural flex row (icon + field + clear button); carries no visual styling of its own |
| `.x-input__field` | Always | The real `<input>` -- border/background/color come from here |
| `.x-input--{xs,sm,md,lg,xl}` | `size` (non-`md`) | Field padding/font-size |
| `.x-input--success` / `.x-input--warning` / `.x-input--error` | `variant` | Field border color |
| `.x-input__prefix` / `.x-input__suffix` | `prefix`/`suffix` (native-input form) | Adornment text |
| `.x-input__clear` | `clearable` | Clear (×) button |
| `.x-input__helper` | `helper` set, no `error` | Helper text |
| `.x-input__error` | `error` set | Error text |
...
```javascript
const field = document.querySelector('x-input input');
