...
<div x-demo>
<div x-alert variant="info" message="A new update is available."></div>
</div>

### Severity Variants

```html
<div x-alert variant="info" title="Heads up" message="This is an informational alert."></div>
<div x-alert variant="success" title="Success" message="Your changes have been saved."></div>
<div x-alert variant="warning" title="Warning" message="Please double-check this value."></div>
<div x-alert variant="error" title="Error" message="Something went wrong."></div>
```
...
```html
<div class="x-alert x-alert--info" role="alert" variant="info">
  <span class="x-alert__icon">ℹ️</span>
  <div class="x-alert__content">
    <div class="x-alert__title">Alert</div>
    <div class="x-alert__message">A new update is available.</div>
  </div>
  <!-- only when dismissible -->
  <button class="x-alert__close">×</button>
</div>
```
...
|-------|--------------|-------------|
| `.x-alert` | Always (also a tag selector: `x-alert`) | Base layout -- flex row, left accent border |
| `.x-alert--info` | `variant="info"` (default) | Blue background/border |
| `.x-alert--success` | `variant="success"` | Green background/border |
| `.x-alert--warning` | `variant="warning"` | Amber background/border |
| `.x-alert--error` | `variant="error"` | Red background/border |
| `.x-alert__icon` | Always | Leading icon |
| `.x-alert__content` | Always | Wraps title + message |
| `.x-alert__title` | `title` set | Bold heading line |
| `.x-alert__message` | Always | Message text |
| `.x-alert__close` | `dismissible` | Close button |
...
Alert colors and structure come from real theme tokens read directly in `src/styles/behaviors/alert.css` (there are no dedicated `--x-alert-*` custom properties in the shipped CSS):

| Variable | Used For | Description |
|----------|----------|--------------|
| `--info-color` / `--info-dark` | `.x-alert--info` | Info background / accent border |
| `--success-color` / `--success-dark` | `.x-alert--success` | Success background / accent border |
| `--warning-color` / `--warning-dark` | `.x-alert--warning` | Warning background / accent border |
| `--danger-color` / `--danger-dark` | `.x-alert--error` | Error background / accent border |
