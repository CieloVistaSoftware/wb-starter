...
| Semantic | `<article>` |
| Root CSS Class | `x-card x-card-expandable` |
...
panel `<div x-demo>` generates automatically from the `events` attribute.

<div x-demo events="wb:cardexpandable:toggle">
<div x-cardexpandable
  title="Read More"
  max-height="80px">
  <p>This is a long content that will be truncated when collapsed...</p>
...
```javascript
const card = document.querySelector('x-cardexpandable');
