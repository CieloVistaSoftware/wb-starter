...
printed live, as JSON, in the log panel `<div x-demo>` generates automatically
from the `events` attribute.

<div x-demo events="wb:table:select">
<table
  selectable
  headers="ID,Name,Role,Department,Location"
  rows='[
...
<table
  ...
  headers="A,B"
  rows="[[1,2],[3,4]]"
  class="x-table--striped x-table--hover"
  x-schema="table">
  <input
    type="search"
...
|-------|--------------|--------------|
| `.x-table` | Always (native `<table>` host only — a `<table>` host is targeted by its own tag) | Base styling |
| `.x-table--striped` | `striped` | Zebra striping |
| `.x-table--hover` | `hover` (default `true`) | Row hover effect |
| `.x-table--bordered` | `bordered` | Cell borders |
| `.x-table--compact` | `compact` | Reduced padding |
...
`.x-table__search` input (see [Searchable](#searchable)). `search(term)`
...
```javascript
const table = document.querySelector('x-table[selectable]');
...
|----------|---------|-------------|
| `--x-table-bg` | `var(--bg-surface)` | Background |
| `--x-table-border` | `1px solid var(--border-color)` | Border style |
| `--x-table-radius` | `4px` | Border radius |
| `--x-table-cell-padding` | `0.75rem 1rem` | Cell padding |
| `--x-table-header-bg` | `var(--bg-secondary)` | Header background |
| `--x-table-header-color` | `var(--text-primary)` | Header text color |
| `--x-table-stripe-bg` | `var(--bg-secondary)` | Stripe background |
| `--x-table-hover-bg` | `var(--bg-tertiary)` | Hover background |
| `--x-table-compact-padding` | `0.5rem 0.75rem` | Compact cell padding |
