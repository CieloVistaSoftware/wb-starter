...
| `video-id` | `string` | `this is the video id` | The 11-character YouTube id — the `v=` value in a watch URL. Use this OR `url`. |
| `controls` | `string` | `this is the controls` | Show YouTube's own player controls. Without them the video can only be driven by script. |
