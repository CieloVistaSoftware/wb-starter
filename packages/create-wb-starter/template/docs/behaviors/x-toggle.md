...
## Description
Distinct from the `switch`/`x-switch` form component under Form Controls — `x-toggle` toggles an arbitrary CSS class on any element, not a form value. Triggers on **mousedown** (not `click`), for immediate visual feedback before the click completes; also responds to Enter/Space when focused.
