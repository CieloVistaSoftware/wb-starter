...
| Root CSS Class | `x-dropdown` (plus `x-dropdown-trigger` on the host itself) |
| Category | Overlay |
...
|-------|-----------|-------------|
| `x-dropdown` | host isn't already a `<div x-dropdown>` tag | Marker class (`x-dropdown.css`'s tag selector covers real `<div x-dropdown>` hosts) |
