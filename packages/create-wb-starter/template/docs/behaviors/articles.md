...
> both `x-article` AND `x-card`/`x-card--auto`/`x-card--hoverable` classes
> at once. Filed as
...
|-------|--------------|-------------|
| `x-articles` | host isn't already a `<div x-articles>` tag | Base marker class |
