 ...
 * §1/§16 — every renderable component example is a live <wb-demo> (renders
 * the control AND shows its source), never a static, non-live code fence.
 * Converted from 37 plain ```html fences to live <wb-demo> blocks (per
 ...
 *
 * This page has MANY <wb-demo> blocks (158 as of this test's writing) on ONE
 ...
 *
 * <wb-demo> (src/wb-viewmodels/wb-demo.js) only builds its first
 ...
 * test scrolls EACH <wb-demo> individually into view via scrollIntoView()
 * before asserting — confirmed live to reliably build all 158.
 */

test.describe('docs/behavior-cross-reference.md: live <wb-demo> examples', () => {
  test('every <wb-demo> renders live grid + source, no page errors', async ({ page }) => {
    ...
    // 158 <wb-demo> blocks as of this test's writing. Assert a floor rather
    // than an exact count so future additions don't need this test touched.
    expect(count).toBeGreaterThanOrEqual(150);

    // Scroll every <wb-demo> individually into view so each one's
    ...
    for (let i = 0; i < count; i++) {
      ...
      // wb-demo's own auto-generated source panel is always `.wb-demo__code`
      // (demo.js: `pre.className = 'wb-demo__code'`) -- NOT a bare `pre`
      // selector. This doc has a "details" example whose rendered content
      // legitimately contains its own <pre><code> inside a collapsed
      // <details> (hidden until expanded); a `.wb-demo__code, pre` selector
      ...
      await expect(demo.locator('.wb-demo__code').first()).toBeVisible();
    }

    expect(errs, 'no page errors while rendering docs/behavior-cross-reference.md').toEqual([]);
  });

  test('the wb-modal dialog trigger pattern actually opens a working dialog', async ({ page }) => {
    ...
    // self-contained `<wb-modal modal-title modal-content>` trigger pattern
    ...
    // un-mangled <wb-modal> tag and actually opens a dialog on click.
    const errs: string[] = [];
    page.on('pageerror', (e) => errs.push(String(e)));
    ...
    const trigger = page.locator('wb-modal', { hasText: 'Show Welcome' }).first();
    ...
    await trigger.click();
    const dialog = page.locator('dialog.wb-dialog');
    ...
    // next assertion.
    await dialog.locator('.wb-dialog__close').click();
    await expect(dialog).toBeHidden({ timeout: 5000 });

    expect(errs, 'no page errors while opening/closing the wb-modal dialog').toEqual([]);
  });
});
