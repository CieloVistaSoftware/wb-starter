 ...
 * "../../demos/behaviors-showcase.html" — correct relative to the doc, but the
 ...
 * SITE_ROOT/demos/behaviors-showcase.html.
 */
test('doc-viewer rebases relative demo links to the doc location (not a domain-root 404)', async ({ page, baseURL }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return !!c && c.querySelector('a[href*="behaviors-showcase"]') !== null;
  }, { timeout: 20000 });

  const link = page.locator('#content a[href*="behaviors-showcase"]').first();
  ...
  expect(new URL(resolved).pathname).toBe('/demos/behaviors-showcase.html');

  // …and that target actually exists (no 404).
  const req = await pwRequest.newContext({ baseURL });
  const res = await req.get('/demos/behaviors-showcase.html');
