 ...
 * a gap" -- a thin visible sliver along a <wb-cardoverlay>'s left/bottom
 ...
 * existed for cropping/aspect-ratio (wb-cardoverlay-not-cropped-narrow) and
...
test.describe('wb-cardoverlay background-repeat stays no-repeat (#635)', () => {
  test('default-variant cardoverlay (the case composeCard\'s shorthand affects) computes background-repeat: no-repeat', async ({ page }) => {
    await renderOverlay(page, `<wb-cardoverlay id="ov" image="https://picsum.photos/seed/regtest635/800/500" title="Test"></wb-cardoverlay>`);
  ...
  test('background-size stays cover and background-image is the real image (no other regression from the fix)', async ({ page }) => {
    await renderOverlay(page, `<wb-cardoverlay id="ov" image="https://picsum.photos/seed/regtest635b/800/500" title="Test"></wb-cardoverlay>`);
  ...
  test('a glass/bordered/flat-variant card (which composeCard does NOT shorthand-reset) is unaffected either way', async ({ page }) => {
    ...
    await renderOverlay(page, `<wb-cardoverlay id="ov" image="https://picsum.photos/seed/regtest635c/800/500" title="Test" variant="glass"></wb-cardoverlay>`);
    const repeat = await page.locator('#ov').evaluate((el) => getComputedStyle(el).backgroundRepeat);
