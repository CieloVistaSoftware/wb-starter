...
/**
 * #669 — <wb-table paginated> must actually paginate.
 *
 * John: "<wb-table paginated ...> where is the pagniation". There was none.
...
test.describe('table: paginated actually paginates (#669)', () => {
  test('the pager renders with real size, not 0x0', async ({ page }) => {
    await render(page, `<wb-table paginated page-size="5" headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);

    const pager = await box(page, '.wb-table__pager');
  ...
  test('Next and Previous move through the pages', async ({ page }) => {
    await render(page, `<wb-table paginated page-size="5" headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);
    const status = () => page.textContent('.wb-table__pager-status');
    ...
    await page.click('.wb-table__pager-btn:last-of-type');
    expect(await status()).toContain('Page 2 of 3');
    expect(await firstVisible()).toBe('row-6');

    await page.click('.wb-table__pager-btn:first-of-type');
    expect(await status()).toContain('Page 1 of 3');
    expect(await firstVisible()).toBe('row-1');
  });
  ...
  test('the final page holds the remainder, and Next is disabled there', async ({ page }) => {
    ...
    expect(await page.isDisabled('.wb-table__pager-btn:last-of-type'), 'Next should be disabled on the last page').toBe(true);
    expect(await page.isDisabled('.wb-table__pager-btn:first-of-type'), 'Previous should be live').toBe(false);
  });

  test('Previous is disabled on the first page', async ({ page }) => {
    await render(page, `<wb-table paginated page-size="5" headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);
    expect(await page.isDisabled('.wb-table__pager-btn:first-of-type')).toBe(true);
  });

  test('pageSize defaults to 10 when unspecified', async ({ page }) => {
    await render(page, `<wb-table paginated headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);
    expect(await visibleRowCount(page), 'the schema default is 10').toBe(10);
    expect(await page.textContent('.wb-table__pager-status')).toContain('Page 1 of 2');
  });

  test('no pager at all without the attribute', async ({ page }) => {
    await render(page, `<wb-table headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);
    expect(await box(page, '.wb-table__pager'), 'pagination is opt-in').toBeNull();
  ...
  test('paginated="false" is off, matching the bare-boolean convention', async ({ page }) => {
    await render(page, `<wb-table paginated="false" headers="Name,N" rows="${rowsAttr(12)}"></wb-table>`);
    expect(await box(page, '.wb-table__pager')).toBeNull();
  ...
  test('the schema spellings hoverable and filterable are honoured', async ({ page }) => {
    ...
    await render(page, `<wb-table hoverable filterable headers="Name,N" rows="${rowsAttr(4)}"></wb-table>`);

    const hasHover = await page.evaluate(() =>
      !!document.querySelector('#harness .wb-table--hover, #harness table.wb-table--hover')
    );
    expect(hasHover, 'hoverable is the documented spelling of hover').toBe(true);

    const search = await box(page, '.wb-table__search');
