 ...
 * live <wb-demo>, so ALL FOUR of this doc's html examples render live, not
 * just the one explicitly wrapped in <wb-demo>.
 *
 ...
 * <wb-demo> repeats the EXACT SAME <wb-drawer-layout> markup already shown
 ...
 * prose right above the first demo ("Wrapped in <wb-demo>, so the live
 * component renders below with its source shown underneath") confirms the
 * Usage section was always redundant -- the wb-demo already shows the exact
...
test.describe('drawer.md: "Drawer Layout" live wb-demo (position=left width=300px)', () => {
  test('renders a real <wb-drawer-layout> with the documented position/width applied', async ({ page }) => {
    const errors = await loadDoc(page);
    const layouts = page.locator('wb-drawer-layout');
    await expect(layouts.first()).toHaveCount(1, { timeout: 5000 }).catch(() => {});
    const count = await layouts.count();
    expect(count, 'no <wb-drawer-layout> rendered at all on drawer.md').toBeGreaterThan(0);
    ...
    expect(width, 'wb-drawer-layout did not apply the documented width').toBe('300px');

    const display = await el.evaluate((e) => getComputedStyle(e).display);
    expect(display, 'wb-drawer-layout behavior did not run (expected display:flex)').toBe('flex');
  ...
  test('sidebar content (h3 + nav) is preserved inside the drawer', async ({ page }) => {
    await loadDoc(page);
    const el = page.locator('wb-drawer-layout').first();
...
test.describe('drawer.md: no redundant duplicate of the same live example (#621)', () => {
  test('the "Drawer Layout" example renders exactly once, not duplicated by a leftover Usage block', async ({ page }) => {
    await loadDoc(page);
    const layouts = page.locator('wb-drawer-layout[position="left"][width="300px"]');
    const count = await layouts.count();
    expect(count, 'the same wb-drawer-layout markup rendered more than once -- a redundant fenced block duplicated the wb-demo above it').toBe(1);
  });
});
...
test.describe('drawer.md: "Drawer Overlay" -- x-drawer button opens a real panel', () => {
  test('clicking the button opens a drawer panel with the documented title and content', async ({ page }) => {
    ...
    const backdrop = page.locator('.wb-drawer__backdrop--open');
...
test.describe('drawer.md: "JavaScript API" snippet -- programmatic drawer() call', () => {
  test('drawer(button, {title, content, position}) matches the documented signature and opens correctly', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const panel = document.querySelector('.wb-drawer__panel--open');
      return {
        panelFound: !!panel,
    ...
    expect(result.position).toContain('wb-drawer--left');
  });
});
