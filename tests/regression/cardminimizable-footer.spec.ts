 ...
 * ReferenceError, not just a lint nit. Any <div x-cardminimizable footer="...">
 ...
 */
test('<div x-cardminimizable footer> renders its footer without throwing (#344)', async ({ page }) => {
  const pageErrors: string[] = [];
  page.on('pageerror', (err) => pageErrors.push(String(err)));

  await page.goto('/');
  await page.setContent(`
    <div x-cardminimizable id="card" title="Test Card" footer="Card Footer">
      Card body content
    </div>
    <script type="module">
  ...
  const footer = page.locator('#card .x-card__footer');
