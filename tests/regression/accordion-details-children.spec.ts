...
async function openStates(page: Page): Promise<boolean[]> {
  return page.evaluate(() => {
    ...
    const items = [...acc.querySelectorAll('details, .wb-accordion__item')];
    return items.map((el) =>
      (el as HTMLDetailsElement).open === true ||
      /(?:^|\s)(?:is-open|wb-accordion__item--open|open)(?:\s|$)/.test(el.className));
  });
}
...
async function clickHeader(page: Page, i: number) {
  const clicked = await page.evaluate((idx) => {
    const acc = document.getElementById('acc');
    if (!acc) return false;
    const heads = [...acc.querySelectorAll('summary, .wb-accordion__header, button')];
