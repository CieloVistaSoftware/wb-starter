 ...
 * <div x-cardoverlay> demo's title text ("City Lights") rendered cropped/cut
 * off at the top of the card, badly overlapping the real caption box
 * ("City Lights" / "Urban photography") below it.
 *
 * Root cause: .x-card--overlay-card (card.css) declares a fixed `height`
 ...
 *
 * Fix: give .x-card--overlay-card a landscape aspect-ratio (with a
...
test.describe('[x-cardoverlay] is not squeezed into an unusably narrow box', () => {
  ...
  test('the real caption (title/subtitle) is fully visible and not covered by the background', async ({ page }) => {
    ...
    const content = page.locator('.x-card__overlay-content');
