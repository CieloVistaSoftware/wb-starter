...
/**
 * #390: <wb-select> silently discarded real <option> children.
...
async function setup(page: Page, html: string): Promise<void> {
  ...
  await page.evaluate((h: string) => {
    ...
    c.id = 'wb-select-option-test-area';
    c.innerHTML = h;
...
test.describe('<wb-select> real <option> children (#390)', () => {
  ...
  test('options="[...]" attribute still works when there are no <option> children (backward compat)', async ({ page }) => {
    await setup(page, `<wb-select id="s3" options='[{"value":"a","label":"Alpha"},{"value":"b","label":"Beta"}]'></wb-select>`);
