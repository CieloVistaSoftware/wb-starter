 ...
 */
test.describe('wb-cardhorizontal image and content never overlap, content is not cut off', () => {
  ...
  test('figure and content boxes do not overlap', async ({ page }) => {
    await setupTestContainer(
      page,
      '<wb-demo columns="1"><wb-cardhorizontal image="https://placehold.co/400x300/0f172a/e2e8f0?text=Web+Components" title="Featured Article" subtitle="The Future of Web Components" content="Web components are evolving rapidly with native browser APIs."></wb-cardhorizontal></wb-demo>'
    );

    const figure = page.locator('.wb-card__horizontal-figure');
    const content = page.locator('.wb-card__horizontal-content');
  ...
  test('title, subtitle, and body text are all fully present and visible, none clipped', async ({ page }) => {
    ...
    const title = page.locator('.wb-card__horizontal-title, .wb-card__title');
    const subtitle = page.locator('.wb-card__subtitle');
    const body = page.locator('.wb-card__horiz-body');
