 ...
 * exact bug with a fenced `<wb-mdhtml src="/docs/guide.md">` example -- once
...
test.describe('mdhtml() autoLiveRender option', () => {
  const ISSUE_BODY_MD =
    ...
    '```html\n' +
    '<wb-mdhtml src="/docs/__auto_live_render_test_placeholder__.md"></wb-mdhtml>\n' +
    '```\n' +
    '\nMore text.';

  test('default (docs) behavior: a real component example gets promoted to a live wb-demo', async ({ page }) => {
    ...
    const result = await page.evaluate(async (md) => {
      ...
      await mdhtml(el, {});
      return { hasLiveDemo: !!el.querySelector('wb-demo wb-mdhtml') };
    }, ISSUE_BODY_MD);
