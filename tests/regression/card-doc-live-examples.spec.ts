...
test.describe('card component documentation examples (#419)', () => {
  test('keeps executable examples live and generated structure explanatory', () => {
    ...
    const demos = [...text.matchAll(/<wb-demo\b[^>]*>([\s\S]*?)<\/wb-demo>/gi)]
      .map((match) => match[1]);
    ...
    expect(
      demos.filter((source) => /<wb-card[\s>]|<article[\s\S]*\sx-card[\s>]/i.test(source)),
    ...
    const structureFence = fences.find(({ source }) => source.includes('wb-card__header'));
