 ...
 * `.wb-card__expandable-content`'s max-height between the collapsed value
 * and 1000px on every click. The bug is that cards.html's demo instance had
 * no `size` attribute, defaulting to `wb-card--auto` ("no constraints,
 * fills container") -- combined with #563's demo-width fix (wb-demo now
 ...
 */
test.describe('wb-cardexpandable actually shows more/less content on click', () => {
  ...
  test('collapsed state clips content shorter than its natural height', async ({ page }) => {
    await setupTestContainer(
      page,
      '<wb-demo columns="1"><wb-cardexpandable size="sm" title="FAQ" subtitle="Click to reveal" content="The card system uses a behavior-based architecture. Each variant is an independent component that composes the shared card structure and CSS — there is no base class to inherit from."></wb-cardexpandable></wb-demo>'
    );

    const content = page.locator('.wb-card__expandable-content');
  ...
  test('clicking the expand button visibly grows the content area', async ({ page }) => {
    ...
    const content = page.locator('.wb-card__expandable-content');
    const btn = page.locator('.wb-card__expand-btn');
