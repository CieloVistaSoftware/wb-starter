...
/**
 * John, live report -- demos/frameworks.html's HTMX section's <wb-demo> code
 ...
 * Svelte, Angular, and Solid, ending in mismatched nested `</wb-demo>` tags.
 *
 * Root cause: src/wb-viewmodels/page-source-cache.js's extractTagBlock()
 * regex-scans the raw page HTML for `<wb-demo ...>...</wb-demo>` occurrences
 ...
 * use <wb-demo> -- prose that happens to contain the literal substring
 * "<wb-demo>" (e.g. "...which <wb-demo>'s HTML-source extraction has no way
 ...
 * literal "</wb-demo>" ANYWHERE later in the file -- landing on the real
 * closing tag of the one actual <wb-demo> (wrapping the HTMX section) and
...
test.describe('demos/frameworks.html: wb-demo code panel is bounded to its own section, not the rest of the page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/demos/frameworks.html');
    // The HTMX section's <wb-demo> is the only one on the page -- wait for
    // its code panel to finish rendering (demo.js builds `.wb-demo__code`
    // asynchronously after measuring/formatting the extracted source).
    await page.waitForSelector('wb-demo .wb-demo__code', { timeout: 30000 });
  });

  test('the HTMX wb-demo code panel contains only the HTMX section\'s own markup', async ({ page }) => {
    const codePanel = page.locator('wb-demo .wb-demo__code').first();
    await expect(codePanel).toContainText('hx-post');
    await expect(codePanel).toContainText('Click Me (HTMX)');
  });

  test('the HTMX wb-demo code panel does NOT swallow other frameworks\' markup', async ({ page }) => {
    const codeText = await page.locator('wb-demo .wb-demo__code').first().innerText();
  ...
  test('the HTMX wb-demo code panel is a short, single-section block, not a multi-thousand-character dump', async ({ page }) => {
    const codeText = await page.locator('wb-demo .wb-demo__code').first().innerText();
