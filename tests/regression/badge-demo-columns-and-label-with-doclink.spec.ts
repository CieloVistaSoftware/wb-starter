 ...
 * 6 <wb-badge> elements, a plain fenced ```html block auto-promoted by
 * mdhtml.js into a live <wb-demo> with no way to ever declare
 ...
 * doc-link 📖 icon (.wb-demo__card-doc-link) is appended as hostEl's DOM
 * CHILD *before* badge() ever runs (attachInstanceDocLink in demo.js), so
 * every wb-demo'd badge saw non-empty textContent from its OWN instrumentation
 ...
 * host with a free corner, but wb-badge is short enough that the icon
 ...
 * `wb-badge:has(> .wb-demo__card-doc-link)` reserves 2.25rem of right
...
test.describe('badge.md "Color Variants": undeclared multi-child wb-demo (#617/#618/#619)', () => {
  const SIX_BADGES = `<wb-demo id="d">
  ...
  test('#617: 6 children with no columns attribute no longer collapse to cols-1', async ({ page }) => {
    ...
    const grid = page.locator('#d .wb-demo__grid');
  ...
  test('#618: every badge still renders its label text alongside the doc-link icon', async ({ page }) => {
    await inject(page, SIX_BADGES);
    const badges = page.locator('#d wb-badge');
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      await expect(badges.nth(i).locator('.wb-demo__card-doc-link')).toHaveCount(1);
    }
  });

  test('#619: the doc-link icon does not visually overlap the label text', async ({ page }) => {
    await inject(page, SIX_BADGES);
    const badge = page.locator('#d wb-badge').first();
    const icon = badge.locator('.wb-demo__card-doc-link');
  ...
  test('regression guard: a genuine single-item demo still shrinks to cols-1 (§7)', async ({ page }) => {
    await inject(page, `<wb-demo id="d"><wb-badge label="Solo" variant="primary"></wb-badge></wb-demo>`);
    const grid = page.locator('#d .wb-demo__grid');
    const className = await grid.getAttribute('class');
  ...
  test('regression guard: an explicit columns attribute is still honored over the new default', async ({ page }) => {
    await inject(
      page,
      `<wb-demo id="d" columns="2">
        <wb-badge label="A" variant="default"></wb-badge>
        <wb-badge label="B" variant="default"></wb-badge>
        <wb-badge label="C" variant="default"></wb-badge>
        <wb-badge label="D" variant="default"></wb-badge>
      </wb-demo>`
    );
    const grid = page.locator('#d .wb-demo__grid');
    const className = await grid.getAttribute('class');
