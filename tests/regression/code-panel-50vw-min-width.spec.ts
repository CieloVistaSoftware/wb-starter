...
/**
 * A single-item demo's `--wb-demo-shrink-width` (demo.js, #486) sizes the
 * WHOLE wb-demo -- code panel included, since `.wb-demo__code` is
 * width:100% of its parent -- to the rendered control's own natural width.
 * For a small control (a plain text <wb-card>, ~250-340px) that left the
 * code panel just as cramped, even though `.wb-demo__code`'s own max-width
 ...
 * mid-word.
 *
 * Fix: `min-width: 50vw` on `wb-demo[data-code-width="50vw"]` itself
...
test.describe('wb-demo[data-code-width="50vw"] actually reaches 50vw', () => {
  test('components.html: the "This is the title" card demo code panel is not cramped', async ({ page }) => {
    ...
    const codePanel = demo.locator('.wb-demo__code');
