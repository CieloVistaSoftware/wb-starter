...
test('wb-progress renders its Light DOM fill after schema-aware scanning', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    container.innerHTML = '<wb-progress id="progress-render-target" value="65" max="100"></wb-progress>';
  ...
  const bar = progress.locator('.wb-progress__bar');
