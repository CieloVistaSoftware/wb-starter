...
/**
 * src/wb-viewmodels/sticky.js (<wb-sticky>, distinct from the `sticky`
 * boolean attribute on wb-header/wb-footer/wb-navbar) uses position:fixed
 ...
 * in a clean, dedicated, fronted tab and every wb-sticky permutation
...
async function ready(page) {
  ...
  await page.waitForSelector('wb-sticky');
  // Let the whole page's layout fully settle (all eager wb-demo blocks
  ...
  // for the first wb-sticky instance specifically (it sits early in the
...
test.describe('wb-sticky', () => {
  test('sticks to the viewport top (position:fixed, top:0) once scrolled past', async ({ page }) => {
    await ready(page);
    const el = page.locator('#sticky-sticky wb-sticky').nth(0); // no offset
  ...
  test('offset="60" reserves 60px from the top instead of 0', async ({ page }) => {
    await ready(page);
    const el = page.locator('#sticky-sticky wb-sticky').nth(1); // offset="60"
    await scrollPastAndSettle(page, el);
    await expect(el).toHaveClass(/is-stuck/);
    const top = await el.evaluate((e) => e.getBoundingClientRect().top);
  ...
  test('animated attribute applies a box-shadow transition once stuck', async ({ page }) => {
    await ready(page);
    const el = page.locator('#sticky-sticky wb-sticky').nth(2); // animated
    await scrollPastAndSettle(page, el);
    await expect(el).toHaveClass(/is-stuck/);
  ...
  test('scrolling back up unsticks and cleans up the placeholder', async ({ page }) => {
    await ready(page);
    const el = page.locator('#sticky-sticky wb-sticky').nth(0);
    await scrollPastAndSettle(page, el);
    await expect(el).toHaveClass(/is-stuck/);
