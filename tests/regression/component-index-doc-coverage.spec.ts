 ...
 *   2. It contains at least one LIVE `<wb-demo>` example with real rendered
 *      content (not an empty/placeholder `.wb-demo__grid`) -- proof the doc
...
// example for mdhtml.js's autoLiveRender to promote into a `<wb-demo>` --
// confirmed by crawling every doc-viewer page in this component index and
// counting `.wb-demo__grid` elements with children. Both are PRE-EXISTING
...
// customized-built-in `<ul is="wb-list">` example is left as a static,
...
//   - docs/components/semantics/list.md  -- example uses `<ul is="wb-list">`
...
test.describe('component index page (#581)', () => {
  ...
  test('clicking a component link navigates to its real, non-stub doc', async ({ page }) => {
    ...
    await expect(page.locator('wb-demo').first().locator('.wb-demo__grid > *').first()).toBeVisible();
  });
});
...
test.describe('every component doc renders real content + a live example (#581)', () => {
  for (const c of INDEX.components) {
    ...
    if (!NO_LIVE_DEMO_DOCPATHS.has(c.docPath)) {
      test(`${c.name} (${c.docPath}): has at least one live wb-demo example`, async ({ page }) => {
        await page.goto(docHref(c.docPath), { waitUntil: 'domcontentloaded' });

        // mdhtml.js promotes eligible ```html fences (or raw <wb-demo> blocks
        // already in the source) into live <wb-demo> elements -- each one
        // builds a `.wb-demo__grid` containing the actual rendered control.
        // Some components upgrade a beat after DOMContentLoaded, so poll.
        await expect
          .poll(
            async () =>
              page.locator('wb-demo .wb-demo__grid').evaluateAll((grids) =>
                grids.some((g) => g.children.length > 0)
              ),
            {
              message: `${c.docPath} should render at least one live wb-demo example with real content`,
              timeout: 15000,
            }
          )
          .toBe(true);
