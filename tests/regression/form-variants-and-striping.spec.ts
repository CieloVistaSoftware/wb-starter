 ...
 *
 * #671: `<textarea variant="error">` applied `wb-textarea--error` correctly and
...
test.describe('striped rows have real contrast (#672)', () => {
  test('both parities are painted, and they differ', async ({ page }) => {
    ...
    await render(page, `<wb-table striped headers="A,B" rows="${rows}"></wb-table>`, ['data.css']);
  ...
  test('the pager uses a defined theme token, not an undefined one', async ({ page }) => {
    ...
    const rows = JSON.stringify(Array.from({ length: 6 }, (_, i) => [`r${i + 1}`, String(i + 1)])).replace(/"/g, '&quot;');
    await render(page, `<wb-table paginated page-size="3" headers="A,B" rows="${rows}"></wb-table>`, ['data.css']);
    const bg = await page.evaluate(() => {
      const el = document.querySelector('.wb-table__pager');
