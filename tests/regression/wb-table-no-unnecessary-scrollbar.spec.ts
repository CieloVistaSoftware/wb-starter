...
/**
 * #589: "wb-table has a small (~8px) horizontal scrollbar on every example
 * in table.md, even when content fits" -- John: "TABLES CANNOT HAVE
 * HORIZONTAL SCROLLBARS UNLESS NECESSARY."
 *
 * Reported live via doc-viewer: every <wb-table> example on
 ...
 * `wb-table, .wb-table { overflow-x: auto; }` means the browser silently
 ...
 * VISIBLE <wb-table> on the page.
 */
test.describe('wb-table never shows an unnecessary horizontal scrollbar (#589)', () => {
  test('docs/components/semantics/table.md: every visible <wb-table> content fits its own box', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(
        scrollWidth,
        `wb-table[${i}] is ${scrollWidth}px of content in a ${clientWidth}px box -- ` +
