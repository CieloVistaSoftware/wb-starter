...
/**
 * REGRESSION (#632): John, live: <wb-button label="Save" icon="💾"> rendered
 ...
 * button.js ever read it. Separately, <wb-button label="Next" icon="→"
...
test.describe('button label attribute renders as text (#632)', () => {
  test('<wb-button label="..." icon="..."> shows both the icon and the label text', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b1" label="Save" icon="💾"></wb-button>`);
    const text = await page.locator('#b1').textContent();
    expect(text?.trim(), 'label text missing from wb-button face').toContain('Save');
    await expect(page.locator('#b1 .wb-button__icon')).toHaveCount(1);
  });
  ...
  test('native <button label="..." icon="..."> shows both the icon and the label text', async ({ page }) => {
    ...
    await expect(page.locator('#b2 .wb-button__icon')).toHaveCount(1);
  });

  test('real text content still wins over label (children win over label)', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b3" label="Ignored">Real Text</wb-button>`);
    const text = await page.locator('#b3').textContent();
    expect(text?.trim()).toBe('Real Text');
  });

  test('label + doc-link icon (wb-demo) coexist -- doc-link does not block label rendering', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b4" label="Save"><a class="wb-demo__card-doc-link">📖</a></wb-button>`);
...
test.describe('button icon-position: both hyphenated and no-hyphen forms work (#632)', () => {
  test('<wb-button icon-position="end"> (correct kebab-case) places the icon after the label', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b5" label="Next" icon="→" icon-position="end"></wb-button>`);
    const html = await page.locator('#b5').innerHTML();
    expect(html.indexOf('wb-button__icon')).toBeGreaterThan(html.indexOf('Next'));
  });

  test('<wb-button iconposition="end"> (no hyphen, as authored) still places the icon after the label', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b6" label="Next" icon="→" iconposition="end"></wb-button>`);
    const html = await page.locator('#b6').innerHTML();
    expect(html.indexOf('wb-button__icon'), 'iconposition (no hyphen) was ignored, icon stayed at the default start position').toBeGreaterThan(html.indexOf('Next'));
  });

  test('default (no icon-position at all) places the icon before the label', async ({ page }) => {
    await renderButtons(page, `<wb-button id="b7" label="Save" icon="💾"></wb-button>`);
    const html = await page.locator('#b7').innerHTML();
    expect(html.indexOf('wb-button__icon')).toBeLessThan(html.indexOf('Save'));
  });
});
