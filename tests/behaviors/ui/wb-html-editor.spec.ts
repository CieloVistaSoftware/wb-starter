...
// Increase timeout for slow CI environments where builder initialization may take longer
...
test.describe('wb-html-editor (Properties panel)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('#canvas .canvas-component', { timeout: 30000 });
  });
  ...
  test('CodeMirror toggle attempts to load (graceful fallback), gutter responsive, and theme toggle works', async ({ page }) => {
    ...
    // Theme toggle exists and toggles theme on the editor
  ...
  test('editing HTML updates canvas in real time (live preview)', async ({ page }) => {
    ...
    const textarea = editor.locator('.wb-html-editor__textarea');
    await textarea.fill('<p test="edited">Live edit!</p>');
    ...
    await expect(page.locator('.canvas-component .component-content [test="edited"]')).toHaveText('Live edit!');
  });
