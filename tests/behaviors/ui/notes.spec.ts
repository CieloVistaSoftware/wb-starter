...
test.describe('Notes Behavior', () => {
  ...
  test.describe('Structure', () => {
    test('should add wb-notes class to element', async ({ page }) => {
      await injectNotes(page, '<div data-wb="notes"></div>');
      const notes = page.locator('#test-container [data-wb="notes"]');
  ...
  test.describe('Open/Close', () => {
    test('should add wb-notes--open class when opened via API', async ({ page }) => {
      await injectNotes(page, '<div data-wb="notes"></div>');
      await page.evaluate(() => {
        const el = document.querySelector('[data-wb="notes"]') as any;
        el.wbNotes.open();
      });
      const notes = page.locator('#test-container [data-wb="notes"]');
