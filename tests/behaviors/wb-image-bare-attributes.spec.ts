/**
 * media.js's image() (x-image) only read data-lazy/data-zoomable, not the
 * bare lazy/zoomable attributes every demo actually uses — the same
...
test.describe('x-image bare zoomable/lazy attributes', () => {
  test('bare zoomable attribute adds wb-image--zoomable', async ({ page }) => {
    await setup(page, `<img id="img1" x-image src="${PIXEL}" zoomable alt="test">`);
    await expect(page.locator('#img1')).toHaveClass(/wb-image--zoomable/);
  });
  ...
  test('without zoomable, wb-image--zoomable is not added', async ({ page }) => {
    await setup(page, `<img id="img3" x-image src="${PIXEL}" alt="test">`);
    await expect(page.locator('#img3')).not.toHaveClass(/wb-image--zoomable/);
  });
});
