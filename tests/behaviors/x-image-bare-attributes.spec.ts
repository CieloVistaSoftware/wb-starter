 ...
 * changed from x-image to x-img to match the file/tag name (audio.js →
...
test.describe('x-image bare zoomable/lazy attributes', () => {
  test('bare zoomable attribute adds x-img--zoomable', async ({ page }) => {
    await setup(page, `<img id="img1" x-image src="${PIXEL}" zoomable alt="test">`);
    await expect(page.locator('#img1')).toHaveClass(/x-img--zoomable/);
  });
  ...
  test('without zoomable, x-img--zoomable is not added', async ({ page }) => {
    await setup(page, `<img id="img3" x-image src="${PIXEL}" alt="test">`);
    await expect(page.locator('#img3')).not.toHaveClass(/x-img--zoomable/);
  });
});
