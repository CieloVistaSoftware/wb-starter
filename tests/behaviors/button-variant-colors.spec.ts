 ...
 *
 * demos/buttons.html (the consolidated button demo, replacing the deleted
 * button-variants-demo.html) uses <wb-button variant="…"> exclusively — the
 * attribute-selector path (button.js: `wb-button[variant="…"]`), not the
...
test.describe('Button variant colors (#button-variant-colors)', () => {
  test('primary/success/error/warning render distinct backgrounds', async ({ page }) => {
    await page.goto('/demos/buttons.html', { waitUntil: 'domcontentloaded' });
