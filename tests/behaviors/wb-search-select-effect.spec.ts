 ...
 * <wb-search> (src/wb-viewmodels/search.js), <wb-select> (src/wb-viewmodels/semantics/select.js),
 ...
 *
 * IMPORTANT — searchable on <wb-select>: demos/site/forms.html itself documents (near its
...
test.describe('<wb-search> effect-based attribute coverage', () => {
  test('instant fires wb:search immediately on input; non-instant is debounced', async ({ page }) => {
    await setup(page, `
      <wb-search id="s-instant" instant placeholder="instant"></wb-search>
      <wb-search id="s-normal" placeholder="normal"></wb-search>
    `);
  ...
  test('loading renders a real spinner element distinct from the non-loading state', async ({ page }) => {
    await setup(page, `
      <wb-search id="s-loading" loading placeholder="loading"></wb-search>
      <wb-search id="s-plain" placeholder="plain"></wb-search>
    `);

    const spinner = page.locator('#s-loading .wb-search__loading');
    await expect(spinner).toHaveCount(1);
    const anim = await spinner.evaluate((el) => getComputedStyle(el).animationName);
    expect(anim).not.toBe('none'); // wb-search-spin keyframe actually applied

    await expect(page.locator('#s-plain .wb-search__loading')).toHaveCount(0);
  });
  ...
  test('variant produces real computed style differences (glass backdrop-filter, minimal border)', async ({ page }) => {
    ...
    // adds the `wb-search--{variant}` class to `element`, which IS the <input> itself --
    ...
    // .wb-search__wrapper, never an ancestor. But search.css's variant rules are all
    // ancestor-descendant selectors (`.wb-search--glass .wb-search__wrapper`,
    // `.wb-search--minimal .wb-search__wrapper`) expecting the modifier class on an
    ...
    // the modifier class to the wrapper (or the host <wb-search> tag) instead of the input.
    test.fail();
    await setup(page, `
      <wb-search id="s-default" placeholder="default"></wb-search>
      <wb-search id="s-glass" variant="glass" placeholder="glass"></wb-search>
      <wb-search id="s-minimal" variant="minimal" placeholder="minimal"></wb-search>
    `);

    const defaultWrapper = page.locator('#s-default .wb-search__wrapper');
    const glassWrapper = page.locator('#s-glass .wb-search__wrapper');
    const minimalWrapper = page.locator('#s-minimal .wb-search__wrapper');
  ...
  test('size sm vs lg produce a real computed height difference', async ({ page }) => {
    ...
    // real run): the `wb-search--sm`/`wb-search--lg` class is added to the <input> element
    // in search.js, before that input is moved inside .wb-search__wrapper -- so it ends up
    // a descendant of the wrapper, not an ancestor. search.css's size rules
    // (`.wb-search--sm .wb-search__wrapper`, `.wb-search--lg .wb-search__wrapper`) need the
    ...
    test.fail();
    await setup(page, `
      <wb-search id="s-sm" size="sm" placeholder="sm"></wb-search>
      <wb-search id="s-lg" size="lg" placeholder="lg"></wb-search>
    `);

    const smHeight = await page.locator('#s-sm .wb-search__wrapper').evaluate((el) => parseFloat(getComputedStyle(el).height));
    const lgHeight = await page.locator('#s-lg .wb-search__wrapper').evaluate((el) => parseFloat(getComputedStyle(el).height));
  ...
  test('disabled blocks real typing -- the underlying input value never changes', async ({ page }) => {
    await setup(page, `<wb-search id="s-disabled" disabled placeholder="disabled"></wb-search>`);
...
test.describe('<wb-select> effect-based attribute coverage', () => {
  const FRUIT_OPTIONS = '[{"value":"a","label":"Apple"},{"value":"b","label":"Banana"},{"value":"c","label":"Cherry"}]';

  test('searchable is currently inert -- no filter UI, identical to a plain <wb-select> (see file header)', async ({ page }) => {
    await setup(page, `
      <wb-select id="sel-plain" options='${FRUIT_OPTIONS}'></wb-select>
      <wb-select id="sel-searchable" searchable options='${FRUIT_OPTIONS}'></wb-select>
    `);
  ...
  test('clearable actually empties the current selection', async ({ page }) => {
    await setup(page, `<wb-select id="sel-clearable" clearable options='${FRUIT_OPTIONS}'></wb-select>`);

    const select = page.locator('#sel-clearable select');
    await select.selectOption('a');
    expect(await select.inputValue()).toBe('a');

    await page.locator('#sel-clearable .wb-select__clear').click();
  ...
  test('multiple allows two options to remain selected simultaneously; default does not', async ({ page }) => {
    await setup(page, `
      <wb-select id="sel-multiple" multiple options='${FRUIT_OPTIONS}'></wb-select>
      <wb-select id="sel-single" options='${FRUIT_OPTIONS}'></wb-select>
    `);
  ...
  test('disabled cannot be changed by interaction', async ({ page }) => {
    await setup(page, `<wb-select id="sel-disabled" disabled options='${FRUIT_OPTIONS}'></wb-select>`);
  ...
  test('required makes the underlying <select> fail native validation when empty, pass once a value is set', async ({ page }) => {
    await setup(page, `<wb-select id="sel-required" required options='${FRUIT_OPTIONS}'></wb-select>`);
...
test.describe('Native <select> and <input x-autocomplete> (forms.html "Select" fallback section)', () => {
  ...
  test('x-autocomplete surfaces a real filtered suggestion for a partial match', async ({ page }) => {
    ...
    const suggestions = page.locator('#test-area .wb-autocomplete__list li');
