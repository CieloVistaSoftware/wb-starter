 ...
 *
 * 2. <textarea>, by contrast, has NO such self-build fallback in
 ...
 *    `<textarea>` stays a plain, non-editable WB-TEXTAREA host: styled,
 ...
 *    <div x-switch label-position>, <div x-switch size>/<div x-switch variant>,
...
async function setup(page: Page, html: string): Promise<void> {
  ...
  await page.evaluate((h: string) => {
    ...
    c.id = 'x-switch-textarea-test-area';
    c.innerHTML = h;
...
async function setupNative(page: Page, html: string): Promise<void> {
  ...
  await page.evaluate((h: string) => {
    ...
    const c = document.createElement('div');
    c.id = 'x-switch-textarea-native-test-area';
    c.innerHTML = h;
    document.body.appendChild(c);
  }, html);
  await page.evaluate(async () => {
...
test.describe('<div x-switch> — real effects (self-build path, #279)', () => {
  test('checked: clicking flips the real state (input.checked + aria-checked), not just a class', async ({ page }) => {
    await setup(page, '<div x-switch id="sw-checked" label="Notify"></div>');
  ...
  test('disabled: clicking does not flip the state', async ({ page }) => {
    await setup(page, '<div x-switch id="sw-disabled" label="Off" disabled></div>');
  ...
  test('SUSPECTED BUG: label-position start/end has no effect on real DOM order', async ({ page }) => {
    ...
    await setup(
      ...
      '<div x-switch id="sw-end" label="End" label-position="end"></div>'
    );
    ...
    const order = await page.evaluate(() => {
      ...
      return {
        startLabelBeforeTrack: indexOf(start, '[class*="x-switch__label"]') < indexOf(start, '.x-switch__track'),
        endLabelAfterTrack: indexOf(end, '[class*="x-switch__label"]') > indexOf(end, '.x-switch__track'),
      };
    });
  ...
  test('SUSPECTED BUG: size produces no computed style difference', async ({ page }) => {
    ...
    await setup(
      ...
      '<div x-switch id="sw-lg" label="B" size="lg"></div>'
    );
    const trackWidth = async (id: string) =>
      page.locator(`#${id} .x-switch__track`).evaluate((el) => getComputedStyle(el).width);
...
test.describe('Native <textarea> section (bare attributes + autoInject, matching forms.html\'s own WB.init config)', () => {
  test('native <textarea autosize>: same real height-growth behavior as the x-behavior path', async ({ page }) => {
    ...
    await expect(ta).toHaveClass(/x-textarea--autosize/);
  ...
  test('native <textarea show-count max-length="100">: counter reflects the real current/max count as you type', async ({ page }) => {
    ...
    const counter = ta.locator('xpath=..').locator('.x-textarea__counter');
