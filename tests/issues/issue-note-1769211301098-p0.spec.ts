...
test.describe('Issue note-1769211301098-p0: Theme Override Not Working', () => {
  ...
  test('changing theme should update CSS variables', async ({ page }) => {
    ...
    // Get initial primary color (accept legacy or canonical variable names)
    const initialColor = await page.evaluate(() => {
      const s = getComputedStyle(document.documentElement);
      return (s.getPropertyValue('--color-primary') || s.getPropertyValue('--primary') || '').trim();
    });
    ...
    // Verify CSS variables updated (colors should be different)
    const newColor = await page.evaluate(() => {
      const s = getComputedStyle(document.documentElement);
      return (s.getPropertyValue('--color-primary') || s.getPropertyValue('--primary') || '').trim();
    });
    
    // Theme change should result in different styling
    // (exact color comparison depends on theme definitions)
    if (initialColor && newColor) {
      expect(newColor).not.toBe(initialColor);
    } else {
      // Ensure at least one theme defines the color variable
