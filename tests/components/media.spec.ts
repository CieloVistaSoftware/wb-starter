...
test.describe('media Behavior', () => {
  ...
  test('renders without errors', async ({ page }) => {
    ...
    const setupHtml = [
      '<wb-media>Test content</wb-media>'
    ];
  ...
  test('element is visible after scan', async ({ page }) => {
    const html = '<wb-media>Test content</wb-media>';
    await injectAndScan(page, html);
    
    const el = page.locator('#test-container wb-media, #test-container [x-media]').first();
