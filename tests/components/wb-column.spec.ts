...
test.describe('wb-column Behavior', () => {
  ...
  test('renders without errors', async ({ page }) => {
    ...
    const setupHtml = [
      '<wb-column>Test content</wb-column>'
    ];
  ...
  test('element is visible after scan', async ({ page }) => {
    const html = '<wb-column>Test content</wb-column>';
    await injectAndScan(page, html);
    
    const el = page.locator('#test-container wb-column, #test-container [x-wb-column]').first();
