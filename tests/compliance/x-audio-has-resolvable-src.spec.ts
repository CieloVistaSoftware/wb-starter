/**
 * COMPLIANCE GATE: every `<audio>`/`<audio>` in the repo must have a
 * resolvable src -- either a `src` attribute on the element itself, or at
 * least one `<source src="...">` child (the standard HTML5 pattern).
 *
 * src/wb-viewmodels/semantics/audio.js throws `x-audio: no src provided --
 ...
 * routinely writes `` `<audio>` `` as prose (naming the tag), and without
 ...
 * `</audio>`/`</audio>` ANYWHERE later in the file -- the same "prose
