 ...
 *
 * §27 of docs/standards/DEMOS-AND-DOCS-STANDARDS.md: a `<wb-demo
 * events="wb:switch:change">` (see src/wb-viewmodels/demo.js) builds a
 * LIVE log panel (`.wb-demo__events-log`) that's supposed to show the
 ...
 * that proof, automated: for every `<wb-demo events="...">` usage in the
 ...
 * replaced with a real `.wb-demo__events-log-entry`).
 *
 ...
 * driving off component-index.json), this greps for `<wb-demo[^>]*\bevents=`
...
function usesLiveEventsDemo(file: string, isMd: boolean): boolean {
  try {
    ...
    return /<wb-demo\b[^>]*\bevents\s*=/.test(live);
  } catch {
    return false;
  }
}
 ...
 * inside one `wb-demo[events]`'s `.wb-demo__grid`. Each entry is a
 ...
 *      `<wb-cardproduct>`'s "Add to Cart" button, `<button x-toggle>`).
 *   2. `[role="switch"]` (covers `<wb-switch>` -- clicking the HOST toggles
 *      it per switch.js, not just the inner hidden checkbox).
 *   3. `[role="tab"]` not already active (covers `<wb-tabs>`; if every tab
 ...
 *      `<wb-table selectable>`'s row-click event).
...
function interactionCandidates(grid: import('@playwright/test').Locator) {
  return [
    { label: 'button (not in a code panel)', locator: grid.locator('button:not(.wb-demo__code button)').first() },
    { label: '[role="switch"]', locator: grid.locator('[role="switch"]').first() },
    { label: '[role="tab"]:not(.wb-tabs__tab--active)', locator: grid.locator('[role="tab"]:not(.wb-tabs__tab--active)').first() },
...
test.describe('wb-demo events= panels actually fire when interacted with (§27 audit)', () => {
  for (const { rel, isMd } of TARGETS) {
    test(`${rel}: every wb-demo[events] logs a real event on interaction`, async ({ page }) => {
      const url = isMd ? '/public/doc-viewer.html?file=' + encodeURIComponent(rel) : '/' + rel;
      await page.goto(url, { waitUntil: 'domcontentloaded' });

      const eventDemos = page.locator('wb-demo[events]');
      ...
      try {
        ...
        test.skip(true, 'no wb-demo[events] rendered on this page (attribute may be inside an inert code sample only)');
      }

      const count = await eventDemos.count();
      if (count === 0) test.skip(true, 'no wb-demo[events] rendered on this page');
      ...
      for (let i = 0; i < count; i++) {
        ...
        const log = demo.locator('.wb-demo__events-log');
        await expect(log, `wb-demo[events][${i}] should build its events log panel (§27)`).toBeAttached({ timeout: 10000 });

        const entriesBefore = await demo.locator('.wb-demo__events-log-entry').count();
        expect(entriesBefore, `wb-demo[events][${i}] should start with an EMPTY event log (nothing interacted with yet)`).toBe(0);
        ...
        for (const { label, locator } of candidates) {
          ...
          const entriesAfter = await demo.locator('.wb-demo__events-log-entry').count();
        ...
        if (!fired) {
          ...
          failures.push(
            `wb-demo[events="${eventsAttr}"][${i}]: interacted with candidate control(s) [${tried.join(', ') || 'none visible'}] ` +
              `but .wb-demo__events-log-entry count never increased -- the event either never fired or the log never caught it`
          );
        }
      }
