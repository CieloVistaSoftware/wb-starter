...
/**
 * COMPLIANCE GATE: every rendered wb-alert (x-alert) must keep DEMOS-AND-
 ...
 * `.wb-alert { padding: 0.75rem 1rem; }` -- top/bottom padding is 0.75rem
...
test.describe('wb-alert (x-alert) keeps >=1rem text-edge padding and inter-alert gap (§13)', () => {
  for (const file of FILES) {
    test(`${file}: wb-alert padding and gaps`, async ({ page }) => {
      ...
      const result = await page.evaluate(({ minPad, minGap }) => {
        const alerts = Array.from(
          document.querySelectorAll('.wb-alert, wb-alert, [x-alert]')
        ) as HTMLElement[];
      ...
      test.skip(result.skip, `${file} has no wb-alert elements`);

      expect(
        result.paddingViolations,
        `${file}: wb-alert padding violation(s):\n  ` + result.paddingViolations.join('\n  ')
      ).toEqual([]);
      expect(
        result.gapViolations,
        `${file}: wb-alert gap violation(s):\n  ` + result.gapViolations.join('\n  ')
      ).toEqual([]);
    });
  }
});
