...
- Rewrote tests/behaviors/fixes-verification.spec.ts wholesale — it bootstrapped via a dead entry point (`src/index.js`, wb-lazy.js) that raced an orphaned WBSite chain; now uses the same `goto('/') + WB.scan()` pattern as permutation-compliance.spec.ts. Fixed ~8 drifted assertions along the way (dead `data-behavior` legacy-syntax assertions, `media.js` split into per-tag files, colorpicker rewritten as a native `type=color` behavior not a custom element, wrong CSS class names).
- `pages/behaviors.html` + `scripts/generate-behaviors-page.js`: fixed 2 compliance violations (`class="wb-btn wb-btn--X"` → `variant="X"`; `<wb-alert>`'s `type=` attribute → `variant=`).
...
**Landed & pushed (8 commits, `845434a`..`ed0b8da`, all on origin/main):**
- `845434a`, `1b6d1aa` — pre-existing local commits, finally pushed (fixes for #372/#374-378, new behavior specs).
- `c64b906` — gitignore `*.crx`/`src/wb-overlay-ext/` (untracked Chrome-extension scaffold, was cluttering `git status`).
- `f627ec7` — batch-1 fixes: cardhero variant= (#383), cardproduct badge= (#380), behaviors-page avatar/skeleton cleanup (#381), the **actual** root cause of #374 (see below), server-side atomic error-log endpoints (#382), button-transition test flakiness (#379).
- `2b3c971` — playground's "Endpoint sees" panel showed unconditionally on every page load/example switch — `.pg-panel`'s own `display:flex` silently beat the browser's default `[hidden]{display:none}` (author CSS always beats user-agent CSS regardless of specificity); this page doesn't load normalize.css's `[hidden]{display:none!important}` safety net. Fixed with a scoped `.pg-panel[hidden]{display:none}` rule.
- `96f8a03` — `semantics/code.js`'s fallback highlight.js theme loader built a cdnjs.cloudflare.com URL from ANY saved theme id, including WB's own local-only themes (e.g. `wb-grayscale-dark`) — confirmed-live 404. Now checks `codecontrol.js`'s `CODE_THEMES` for a local `path` first.
- `3a84489` — `wb-search` host element (`<wb-search variant=...>`) never got its own variant/size classes, only its inner `<input>` did (#359).
- `ed0b8da` — **the reframe**: deprecated `wb-checkbox`/`wb-textarea` (schema-\$view wrappers around native elements whose behavior functions already fully enhance the bare element), added `role="switch"` to `wb-switch`'s real checkbox input (free native affordance), added a native `autocomplete` default to `wb-password`.

**The real #374 root cause (bigger than the original lightbox report):** `<wb-demo>` blocks past `EAGER_BUILD_COUNT` (5) build lazily via `IntersectionObserver`, well after the page's one-time eager `WB.scan(document.body)` already ran. `demo.js` only re-scanned its own source-code `<pre>` panel, never the real children it moves into `.wb-demo__grid` — so ANY interactive element inside a lazy-built demo block never got its behavior attached, not just the lightbox. This explains the vast majority of the historical 234-240 `behaviors` project test failures (#354/#341). Fixed via an `isLazy` flag threaded from `wb-demo.js`'s observer callback into `demo.js`, which now scans the grid only on that path (the eager path's existing scan-race-avoidance is untouched). New general regression test (`lazy-demo-behavior-injection.spec.ts`) verified to actually catch the regression (reverted the fix locally, confirmed it goes red).

**Architectural finding (not yet acted on beyond the checkbox/textarea case):** `wb.js` and `wb-lazy.js` are NOT a shared core with a lazy-mode flag — they are two independently-maintained, duplicated implementations of overlapping responsibility (DOM scanning, behavior dispatch, IntersectionObserver lazy-loading). Confirmed via: two independently-hardcoded version strings that had drifted (`3.0.0` vs `2.1.0`, fixed as part of #371), different `scan()` signatures, and `wb-lazy.js` having zero schema-\$view support at all while `wb.js` has it fully. This is the root architectural cause behind #371, #322, and the checkbox/textarea gap. Whether to eventually unify the two engines (vs. just keep deprecating native-duplicate components one at a time) is an open question for John, not decided this session.

**Issues closed this session:** #372, #374, #375, #376, #377, #378, #380, #381, #383, #359, #301 (all with commit + verification referenced in their closing comments). #386 opened-then-closed same session (wrong initial diagnosis — see #387 for the corrected direction).

**Issues fixed but left open pending full verification (don't re-diagnose, just verify):**
- **#382** — error-log race condition, real root cause fixed (server-side atomic append/clear endpoints replacing the old blind-overwrite pattern). Needs verification under actual 8-worker CI load, not just an isolated run.
- **#379** — button-attributes-effect.spec.ts flakiness, real root cause fixed (`.wb-button`'s `transition:all` was letting the test read font-size mid-transition; test now disables transitions in its fixture). Needs the same 8-worker load verification.
- **#371** — version-drift part fixed (`wb-lazy.js` was hardcoded to `2.1.0`, now `3.0.0`). Image-retry-storm part narrowed (ruled out picsum.photos being slow) but NOT fixed — needs live browser tracing of `card.js`'s "STALE CHECK"/`media-load-retry.js`'s ready-detection logic.
- **#354, #341** — both are the same systemic issue as #374, now fixed there. Needs a fresh CI run against current `main` to confirm the historical 234-240 failure count is actually gone (last real CI run predates all of tonight's fixes).
- **#365** — priority finding (wb-skeleton) was a FALSE POSITIVE in the original audit's methodology (skeleton.css is intentionally tag-selector-only, "empty className" isn't a broken signal there) — corrected in the issue comment, no code fix needed for it specifically. The other 19 backlogged items in that issue are unaffected, still stand as originally triaged (never-used-as-own-tag, low priority).
- **#322** — reproduced live (doc-viewer `pre` elements never get `x-behavior="pre"` applied), but two hypotheses were ruled out (not an eager/lazy scan default issue — even an explicit `WB.scan(pre,{eager:true})` doesn't fix it; not a class-name mismatch). Root cause is upstream in `WB.inject()`'s own dynamic-import/pending-tracking path on `wb-lazy.js` specifically — needs tracing there, not scan()'s entry point.

**New issues filed this session (none started):**
- **#384** — home page's hero doesn't demonstrate the actual "one script does it all" pitch (real `index.html` loads 4+ CSS files, a schema-driven site-engine architecture — nowhere near the two-stylesheet-one-script pitch). John responded "I really like this hero page" re: a *different* screenshot (the "Build stunning UIs / 100 COMPONENTS" hero) — worth checking whether that's actually what's live or a mockup/other project before doing anything with #384.
- **#385** — wb-demo blocks show how to wire up a control, never how to listen for its events. Need a new docs section for real `addEventListener('wb:...')` examples.
- **#387** — audit the rest of the ~90 schema-driven components (schema-builder.js's `SCHEMA_EXCLUDED_TAGS` lists the ~26 already self-sufficient ones) for the same wb-checkbox/wb-textarea anti-pattern. `wb-dialog` flagged as the best next candidate (builds a div-based modal instead of using the real native `<dialog>` element).

**Also raised by John, not yet actioned:**
- Playground's Examples dropdown needs many more examples, including composition examples (a page with links, a page for a whole site, etc.) — screenshot annotation, no issue filed yet.
- `demos/playground.html`'s hero typewriter effect (`x-typewriter` typing out `<button x-ripple x-tooltip="Click me">Ship it</button>` as decorative text) looks exactly like a live interactive demo but isn't one — John was asked which fix he wanted (make it a real live button / label it as example text / leave it) and dismissed the question without choosing — still unresolved, don't guess.
- `CieloVistaSoftware/wb-behaviors` (separate GitHub repo) validated as safe to delete (created+abandoned same day, 7+ months untouched, zero real code dependency anywhere) — John needs to run the deletion himself (`gh repo delete CieloVistaSoftware/wb-behaviors --yes`), not something I can do. The one live reference (wb-starter's own README "powered by WB Behaviors" link) will need updating if/when he deletes it — not done yet, waiting on him.

**Next step (pick up here):**
1. Finish the #387 audit — go through the remaining schema-driven components looking for the checkbox/textarea pattern. Start with `wb-dialog` (flagged as the clearest next candidate).
2. Trigger a fresh full `npm test`/CI run against current `main` and update #354/#341/#382/#379 with real numbers — the last real data point is the stale 234-240-failure run from before any of tonight's fixes landed.
3. Trace #322's actual root cause inside `WB.inject()`'s dynamic-import path on `wb-lazy.js`.
4. Decide (with John) whether to unify `wb.js`/`wb-lazy.js` into one engine, or keep deprecating native-duplicate components piecemeal — the architectural finding above is real but nothing's been decided on it yet.
5. Untouched from the original 44-issue list, still open, not yet even triaged: #340 (V3-STANDARDS.md rewrite), #324/#323 (frameworks.html/behaviors-reference.md missing wb-demo examples), #307 (Rule 4 compliance gate), #304 (behaviors page mixing wb-*/x-* demos), #295/#294 (docs text-wrap / rem-not-px audits), #291/#287/#286/#283/#274/#273/#272/#268/#265/#253/#243/#224, and #370/#344/#352/#351/#350 (previously assessed as correctly-scoped backlog/blocked-on-repro, no new info this session).

**Open questions:** wb.js/wb-lazy.js unification decision (see above); what to do with the typewriter hero example (John dismissed the question, needs re-asking or a different framing); whether #384's "I really like this hero page" screenshot is describing a target to build toward or something already built elsewhere.
