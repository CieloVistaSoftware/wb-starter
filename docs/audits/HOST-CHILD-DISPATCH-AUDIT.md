...
| `<input>` | same-name self-collision, no host guard at all — dispatching `input()` on the `<input>` host wrapped/styled the whole already-built component as if it were a bare input; separately, `placeholder`/`value`/`name`/`type` never reached the real built `<input>` at all; a duplicate clear button was a third symptom of the same root cause | ✅ Fixed — [#367](https://github.com/CieloVistaSoftware/wb-starter/issues/367), closed |
