...
|-----------|------|---------|-------------|
| `layout` | string | `"grid"` | `grid`, `list`, or `masonry` — sets `.wb-articles--{layout}` on the inner list wrapper |
| `columns` | number | `"3"` | Column count for `grid`/`masonry` layouts, applied as the `--wb-articles-columns` CSS custom property |
| `title` | string | `""` | Optional heading rendered above the list in a `.wb-articles__header` |
...
| `.wb-articles__pagination` | `pagination` | Wraps the Previous/label/Next controls |
| `.wb-articles__page-label` | `pagination` | The static "Page 1" text |
