...
## Description
Distinct from the `switch`/`<div x-switch>` form behavior under Form Controls — `x-toggle` toggles an arbitrary CSS class on any element, not a form value. Triggers on **mousedown** (not `click`), for immediate visual feedback before the click completes; also responds to Enter/Space when focused.
