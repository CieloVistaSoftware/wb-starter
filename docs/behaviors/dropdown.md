...
| `label` | string | `""` | Trigger button text. If set (or the host has real child items), a dedicated `<button class="wb-dropdown__trigger">` is built; otherwise the host's own text content is the clickable trigger |
...
`src/styles/behaviors/dropdown.css`'s own `.wb-dropdown-menu`/`.wb-dropdown-item`
selectors (singular hyphen) are stale and don't match anything this behavior
actually creates (it builds `wb-dropdown__menu`/`wb-dropdown__item`, BEM-style).
...
| `.wb-dropdown__item` | each menu item (generated from `items`, or moved-in child elements) | `role="menuitem"` |
