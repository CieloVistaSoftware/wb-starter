...
```html
<div class="wb-textarea-wrapper">
  <textarea
    class="wb-textarea wb-textarea--autosize"
    placeholder="Grows as you type...">
  </textarea>
  <div class="wb-textarea__counter">0/200</div>
</div>
```
...
|-------|--------------|-------------|
| `.wb-textarea` | Always | Border/background/color/resize behavior |
| `.wb-textarea--{variant}` | `variant` (non-`default`) | Border color |
| `.wb-textarea--{xs,sm,lg,xl}` | `size` (non-`md`) | Padding/font-size (shares the scale with `.wb-input--*`/`.wb-select--*`) |
| `.wb-textarea--autosize` | `autosize` | Disables manual resize; height is JS-managed |
| `.wb-textarea--has-counter` | `show-count` | Marks the field as counter-wrapped |
| `.wb-textarea-wrapper` | `show-count` | Structural wrapper around field + counter |
| `.wb-textarea__counter` | `show-count` | Live character count text |
