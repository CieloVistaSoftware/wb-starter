...
function makeIdPrefix(relPath) {
  // sanitize path to a short prefix
  const name = relPath.replace(/[\\/]/g, '-').replace(/[^a-zA-Z0-9\-]/g, '');
  return `auto-${name}`;
}
...
function processFile(filePath) {
  ...
  const stack = [];
  const toAdd = []; // {openStart, insertAt, id}

  // helper to get attribute id presence
  ...
  for (let i=0;i<tokens.length;i++) {
    ...
    if (t.isClosing) {
      ...
      while (popped && popped.tag !== t.tag) {
        popped = stack.pop();
      }
      if (popped) {
        // decide on missing id and >1 children
        if (!popped.hasId && popped.childCount > 1 && !SKIP_TAGS.has(popped.tag)) {
          toAdd.push({ openStart: popped.openStart, openEnd: popped.openEnd, tag: popped.tag });
        }
        // count as child of current top
        const parent = stack[stack.length-1];
        if (parent) parent.childCount++;
      }
    } else {
      // opening tag
      // ignore void/self-closing tags as they don't have children
      if (t.selfClose) {
        // increment child count of parent
        if (stack.length>0) stack[stack.length-1].childCount++;
        continue;
      }
      const node = { tag: t.tag, openStart: t.start, openEnd: t.end, attrs: t.attrs, hasId: hasId(t.attrs), childCount: 0 };
      stack.push(node);
    }
  }
  ...
  // Build new content with inserted ids
  let out = '';
  let lastIndex = 0;
  let idCounter = 1;
  const prefix = makeIdPrefix(relPath);

  // Sort toAdd by openStart ascending
  toAdd.sort((a,b)=>a.openStart - b.openStart);

  for (const a of toAdd) {
    // find the exact opening tag text to insert into (between '<tag' and '>')
    const openTag = raw.slice(a.openStart, a.openEnd);
    // insert id before the closing '>' of the opening tag, but after other attrs
