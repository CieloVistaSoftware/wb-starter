...
// Fails if PR modifies files under docs/ but the changed docs are not referenced
// in docs/_today/TODO.md or docs/_today/CURRENT-STATUS.md.
...
// Determine changed files against main
...
// For each changed docs file require that its filename or directory is referenced in docs/_today
const missing = [];
for (const f of docsChanged) {
  const basename = path.basename(f);
  const short = f.replace(/^docs\//, '');
  // Accept either the basename or the short path or the PR number being mentioned
