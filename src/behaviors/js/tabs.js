...
export function tabs(element, options = {}) {
  ...
  // Extract tab data - support both data-tab and data-tab-title
  const tabData = panels.map((panel, i) => ({
    title: panel.dataset.tabTitle || panel.dataset.tab || `Tab ${i + 1}`,
  ...
  // Build tabs UI
  element.innerHTML = `
    <nav class="wb-tabs__nav" role="tablist" style="
      ...
      ${tabData.map((tab, i) => `
        <button class="wb-tabs__tab ${i === 0 ? 'wb-tabs__tab--active' : ''}" 
                ...
                style="
                  ...
                  cursor:pointer;color:inherit;font-size:0.8rem;
    ...
    <div class="wb-tabs__panels">
      ${tabData.map((tab, i) => `
        <section class="wb-tabs__panel" role="tabpanel" data-index="${i}" 
             id="panel-${i}"
