 ...
 * 
 * Usage:
 *   <div data-wb="codecontrol"></div>
 *   <div data-wb="codecontrol" data-size="xs"></div>
 *   <div data-wb="codecontrol" data-show-label="false" data-size="sm"></div>
 */
...
const SYNC_EVENT = 'wb:codetheme:sync';
...
export function codecontrol(element, options = {}) {
  ...
  element.classList.add('wb-codecontrol');

  // Create the control UI
  const wrapper = document.createElement('div');
  wrapper.className = 'wb-codecontrol__wrapper';
  ...
  if (config.showLabel) {
    ...
    label.className = 'wb-codecontrol__label';
  ...
  select.className = 'wb-codecontrol__select';
  ...
  if (config.persist && typeof localStorage !== 'undefined') {
    const saved = localStorage.getItem('wb-code-theme');
  ...
  const applyTheme = (themeId, broadcast = true) => {
    ...
    // Persist if enabled
    if (config.persist && typeof localStorage !== 'undefined') {
      localStorage.setItem('wb-code-theme', themeId);
    }

    // Dispatch change event
    element.dispatchEvent(new CustomEvent('wb:codetheme:change', {
      bubbles: true,
      detail: { 
        theme: themeId, 
  ...
  // Cleanup
  return () => {
    element.classList.remove('wb-codecontrol');
