 ...
 */
export function navbar(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
  ...
  if ((config.items.length > 0 || config.brand || config.logo) && existingChildren.length === 0) {
    ...
    element.querySelectorAll('a, []').forEach(link => {
      ...
      if (link instanceof HTMLElement && !link.classList.contains('wb-navbar__brand')) {
    ...
    if (list instanceof HTMLElement) {
      ...
      links.forEach(link => {
        ...
        link.addEventListener('mouseenter', () => { if (link instanceof HTMLElement) link.style.opacity = '1'; });
        link.addEventListener('mouseleave', () => { if (link instanceof HTMLElement) link.style.opacity = '0.8'; });
      });
    }
 ...
 */
export function sidebar(element: HTMLElement, options: Record<string, any> = {}) {
  // Initial config
  let config = {
    items: (options.items || element.dataset.items || '').split(',').filter(Boolean),
 ...
 */
export function menu(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
    items: (options.items || element.dataset.items || '').split(',').filter(Boolean),
    ...options
  };
 ...
 */
export function pagination(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 * Custom Tag: <wb-steps>
 */
export function steps(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
    items: (options.items || element.dataset.items || '').split(',').filter(Boolean),
    current: parseInt(options.current || element.dataset.current || '1'),
    ...options
 ...
 */
export function treeview(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function backtotop(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function link(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function statusbar(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
    items: (options.items || element.dataset.items || '').split(',').filter(Boolean),
