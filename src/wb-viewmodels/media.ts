 ...
 */
export function image(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function gallery(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 * Custom Tag: <wb-video>
 */
export function video(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function audio(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
    src: options.src || element.dataset.src || '',
    controls: options.controls ?? element.dataset.controls !== 'false',
    ...
    if (enableEq) {
      ...
      audioEl.addEventListener('error', (e) => {
        ...
        if (audioEl.crossOrigin && audioEl.error && (audioEl.error.code === 4 || audioEl.error.code === 2)) {
          ...
          audioEl.load();
          if (config.autoplay) audioEl.play().catch(e => console.warn('Autoplay failed after fallback', e));
        }
      }, { once: true });
    }
    ...
    Object.keys(presetData).forEach(name => {
      ...
      btn.onclick = () => {
        ...
        presetData[name].forEach((presetVal, i) => {
          if (filters[i]) filters[i].gain.value = presetVal;
 ...
 */
export function youtube(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 * Vimeo - Vimeo embed
 */
export function vimeo(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
    id: options.id || element.dataset.id,
    autoplay: options.autoplay ?? element.hasAttribute('data-autoplay'),
    muted: options.muted ?? element.hasAttribute('data-muted'),
 ...
 */
export function ratio(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
 ...
 */
export function figure(element: HTMLElement, options: Record<string, any> = {}) {
  const config = {
