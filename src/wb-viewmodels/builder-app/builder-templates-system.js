...
export function addTemplateHTML(html, templateMeta = {}, options = {}) {
  ...
  if (toast) toast('Added ' + (templateMeta.name || 'Template'));
}
...
export function addTemplate(template, options = {}) {
  ...
  if (!template || !template.components) {
    if (toast) toast('Invalid template');
    return;
  }
  ...
  if (toast) toast(`Added "${template.name}" template`);
  renderTree();
  if (saveHist) saveHist();
  updateBadges();
}
...
<body>
  ...
  if (win) {
    ...
    if (toast) toast('Please allow popups to preview');
  }
}
    ...
    if (e.data?.type === 'useTemplate' && e.data?.id) {
      fetch('/data/templates.json')
        ...
        .then(data => {
          ...
          if (template && window.addTemplate) window.addTemplate(template);
        });
    }
  });
}
