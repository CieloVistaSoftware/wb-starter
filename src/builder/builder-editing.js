...
export const CONTAINER_COMPONENTS = {
  ...
  grid: {
    dropZone: '[data-wb="grid"]',
    accepts: ['all'],
    rejects: [],
    childBehavior: 'append'
  },
  flex: {
    dropZone: '[data-wb="flex"]',
    accepts: ['all'],
    rejects: [],
    childBehavior: 'append'
  },
  stack: {
    dropZone: '[data-wb="stack"]',
    accepts: ['all'],
    rejects: [],
    childBehavior: 'append'
  },
  container: {
    dropZone: '[data-wb="container"]',
    accepts: ['all'],
    rejects: [],
    childBehavior: 'append'
  },
  fieldset: {
    dropZone: '[data-wb="fieldset"]',
    accepts: ['all'],
    rejects: ['modal', 'drawer'],
    childBehavior: 'append'
  }
...
export function findDropZone(containerWrapper, containerConfig) {
  const wbEl = containerWrapper.querySelector('[data-wb]');
  if (!wbEl) return null;
...
export function addResizeHandle(wrapper) {
  ...
  presets.innerHTML = `
    <button data-flex="row" title="Row Layout">Row</button>
    <button data-flex="column" title="Column Layout">Col</button>
    <div style="width: 1px; height: 12px; background: rgba(255,255,255,0.2); margin: 0 2px;"></div>
    <button data-width="calc(25% - (var(--gap, 0px) * 0.75))" title="1/4 width">25%</button>
    <button data-width="calc(33.333% - (var(--gap, 0px) * 0.666))" title="1/3 width">33%</button>
    <button data-width="calc(50% - (var(--gap, 0px) * 0.5))" title="1/2 width">50%</button>
    <button data-width="calc(75% - (var(--gap, 0px) * 0.25))" title="3/4 width">75%</button>
    <button data-width="100%">100%</button>
    <button data-width="auto">Auto</button>
  `;
...
export function initKeyboardMove(canvas) {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const component = selected.querySelector('[data-wb]') || selected;
...
export function initSnapGrid(canvas) {
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    let targetEl = null;
    const cardDraggable = e.target.closest('[data-wb="carddraggable"]');
    ...
    if (cardDraggable) {
      ...
      targetEl = wrapper.querySelector('[data-wb]') || wrapper.firstElementChild || wrapper;
    } else {
      return;
    }
