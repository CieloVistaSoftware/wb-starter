...
function require(condition, message, context = {}) {
  if (!ContractConfig.enabled) return;
...
function checkAllInvariants() {
  ...
  if (BuilderState._selectedComponent) {
    const exists = BuilderState._components.some(
      c => c && c.id === BuilderState._selectedComponent.id
    );
    if (!exists) {
...
function assertInvariants(afterOperation = 'unknown') {
  if (!ContractConfig.enabled || !ContractConfig.checkInvariantsOnMutation) return;
...
const BuilderContracts = {
  ...
  component: {
    ...
    ensureDataSet: (id, key, expectedValue) => {
      ...
      const actualValue = comp?.data?.[key];
        ...
        `Component data.${key} should be ${expectedValue}, got ${actualValue}`, 
        { id, key, expectedValue, actualValue });
    }
  },
  ...
  dom: {
    ...
    ensureAttributeSet: (el, attr, value) => {
      const actual = el?.getAttribute?.(attr);
        ...
        `Attribute ${attr} should be "${value}", got "${actual}"`, 
        { attr, expected: value, actual });
    }
  },
  ...
  page: {
    ...
    ensureIsCurrent: (id) => {
      ensure(BuilderState?._currentPageId === id, 
        `Page should be current: ${id}`, 
        { id, actual: BuilderState?._currentPageId });
    }
  },
