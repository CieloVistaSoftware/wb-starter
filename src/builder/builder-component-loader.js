...
const SCHEMA_PATH = 'src/behaviors/schema';
...
export async function loadComponentsFromSchemas() {
  ...
  try {
    ...
    for (const filename of schemaIndex) {
      ...
      if (filename.includes('.base.') || filename.startsWith('_')) continue;
...
async function fetchSchemaIndex() {
  ...
  try {
    const response = await fetch('data/schema-manifest.json');
    if (response.ok) {
...
async function fetchSchema(filename) {
  const response = await fetch(`${SCHEMA_PATH}/${filename}`);
...
export async function generateSchemaManifest() {
  ...
  fs.writeFileSync(
    path.join(process.cwd(), 'data/schema-manifest.json'),
